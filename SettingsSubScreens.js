...
export function SettingsLanguagePage({ navigation, route }) {
  ...
  const onPick = async (id) => {
    ...
    const nextUser = s?.user ? { ...routeUser, ...s.user } : routeUser;
    /** Залишаємось на цьому екрані: одразу оновлюємо підписи; «Назад» → Settings уже з новою мовою зі сховища. */
...
export function SettingsGeoPage({ navigation, route }) {
  ...
  const [geoInitial, setGeoInitial] = useState(true);
  ...
  const refreshGeo = useCallback(async () => {
    try {
      ...
      setServicesOn(!!svc);
      setPermStatus(fg?.status ?? null);
      setPermCanAskAgain(fg?.canAskAgain !== false);
    } catch (_) {
      setServicesOn(null);
      setPermStatus(null);
    } finally {
  ...
  const requestForeground = useCallback(async () => {
    ...
    try {
      ...
      setPermStatus(r?.status ?? null);
      setPermCanAskAgain(r?.canAskAgain !== false);
      setServicesOn(await Location.hasServicesEnabledAsync());
    } catch (_) {
      /* ignore */
    } finally {
      setGeoBusy(false);
        ...
        return (
          <View style={light ? styles.lightList : styles.darkListWrap}>
            ...
            <View style={[styles.geoRingOuterCard, { backgroundColor: borderColor }]}>
              ...
              >
                {geoInitial ? (
                  <View style={[styles.notifRow, styles.notifRowSingle, { justifyContent: 'center' }]}>
                    <ActivityIndicator color={light ? BRAND_BLUE : ACCENT} style={{ paddingVertical: 16 }} />
                  </View>
                ) : (
                  <>
            ...
            {geoBusy ? (
              <ActivityIndicator color={light ? BRAND_BLUE : ACCENT} style={{ marginTop: 16, marginBottom: 8 }} />
            ) : null}
          </View>
        );
      }}
