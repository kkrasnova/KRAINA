...
function warn(variable: string, issue: string, hint: string): void {
...
export function runStartupChecks(): void {
  ...
  if (isProd) {
    ...
    if (
      ...
      warn(
        'DATABASE_URL',
        'points_to_localhost',
        'Production deployments must use a managed PostgreSQL host, not localhost.',
      );
    }
  }
  ...
  if (config.jwtSecret.length < 32) {
    warn(
      'JWT_SECRET',
      'too_short',
      `JWT_SECRET is ${config.jwtSecret.length} chars. Use at least 32 random characters (64+ recommended) in production.`,
    );
  }
  if (config.refreshPepper.length < 32) {
    warn(
      'REFRESH_SECRET',
      'too_short',
      `REFRESH_SECRET is ${config.refreshPepper.length} chars. Use at least 32 random characters (64+ recommended) in production.`,
    );
  }
  ...
  const pubUrlIsLocal =
    pubBaseUrl.includes('localhost') || pubBaseUrl.includes('127.0.0.1');

  if (isProd && pubUrlIsLocal) {
    warn(
      'PUBLIC_BASE_URL',
      'is_localhost',
      'Set PUBLIC_BASE_URL to the public HTTPS URL of this API (e.g. https://api.yourdomain.com). ' +
  ...
  const hasFirebasePath =
    (process.env.FIREBASE_SERVICE_ACCOUNT_PATH ?? '').trim().length > 0;

  if (!hasFirebaseJson && !hasFirebasePath) {
    const msg =
      'Firebase custom tokens will not be issued and Firestore follow validation is disabled. ' +
  ...
  if (isProd && !config.trustProxy) {
    warn(
      'TRUST_PROXY',
      'not_set_in_production',
      'Set TRUST_PROXY=1 when running behind a reverse proxy (nginx, ALB, Traefik). ' +
