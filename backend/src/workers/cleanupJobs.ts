...
function parseIntervalMs(envName: string, defaultMs: number): number {
 ...
 */
async function cleanExpiredRefreshTokens(): Promise<void> {
 ...
 */
async function cleanExpiredPasswordResetTokens(): Promise<void> {
  const r = await pool.query(
    `WITH deleted AS (
 ...
 */
async function auditNotificationRetention(): Promise<void> {
  const r = await pool.query(
 ...
 */
function makeScheduled(jobName: string, fn: () => Promise<void>): () => void {
  let running = false;
  return () => {
    if (running) {
...
const _handles: ReturnType<typeof setInterval>[] = [];
...
export function startCleanupJobs(): void {
  ...
  const jobs: Array<{ name: string; fn: () => Promise<void>; intervalMs: number }> = [
    {
      name: 'refresh_tokens',
