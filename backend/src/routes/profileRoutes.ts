...
import {
  ...
  getPublicProfileFullByUsername,
  searchProfilesForViewer,
...
router.patch('/me', authenticateToken, withIdempotency, async (req, res, next) => {
  try {
    ...
    if (!parsed.success) {
      ...
      if (key === 'bio' && z.code === 'too_big') code = 'bio_too_long';
      else if (key === 'username') code = 'username_invalid';
      else if (key === 'birth_date') code = 'invalid_birth_date';
      else if (key === 'display_name' && z.code === 'too_big') code = 'display_name_too_long';
      else if (key === 'location_label' && z.code === 'too_big') code = 'location_label_too_long';
      else if (key === 'firebase_uid') code = 'firebase_uid_invalid';
