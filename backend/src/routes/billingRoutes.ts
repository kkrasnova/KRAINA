...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!userId) throw new HttpError(401, 'token_invalid');
      ...
      const userEmail =
        emailRow.rows[0] && typeof (emailRow.rows[0] as { email?: string }).email === 'string'
          ? String((emailRow.rows[0] as { email: string }).email)
          : null;
  ...
  async (req, res, next) => {
    try {
      ...
      if (!parsed.success) {
        ...
        const code =
          typeof issue?.message === 'string' && issue.message.length < 80
            ? issue.message
            : 'invalid_body';
