...
function extFromUrlOrType(url: string, contentType: string) {
...
export function isAlreadyHostedLandmarkMedia(url: string) {
  const u = String(url || '');
...
export function isNonPhotoJunkUrl(url: string) {
  const u = String(url || '').toLowerCase();
  if (!u) return true;
  return /\/icon|icon_|_icon|logo|flag|map_|_map|locator|symbol|coat.?of.?arms|emblem|diagram|floor.?plan|site.?plan|wikidata|qr.?code|\.svg($|\?)|signage|plaque|infobox|commons-logo|edit.?button|speaker.?icon|padlock/i.test(
    u,
  );
}

/** Reject war / scaffold / ruins URLs for hero & card covers. */
export function isUnattractiveLandmarkPhotoUrl(url: string) {
  const u = String(url || '').toLowerCase();
  return /scaffold|scaffolding|destroyed|destruction|ruin|rubble|damage|damaged|war_|_war|bombard|missile|shelling|fire_damage|burned|burnt|collapse|construction_fence|covered_in|netting|ukraine.?war|russian.?invasion|руїн|разруш|обстріл|обстрел|риштуван/i.test(
    u,
  );
}

/** Reject icons, maps, diagrams and other non-photo junk for story pages. */
export function isJunkLandmarkPhotoUrl(url: string) {
  const u = String(url || '').toLowerCase();
  if (!u) return true;
  if (isUnattractiveLandmarkPhotoUrl(u)) return true;
  return isNonPhotoJunkUrl(u);
}

/** True when URL looks like war / damage / ruin documentation (for story pages). */
export function isDocumentaryDamagePhotoUrl(url: string) {
  const u = String(url || '').toLowerCase();
  return /destroyed|destruction|ruin|rubble|damage|damaged|war_|_war|bombard|missile|shelling|fire_damage|burned|burnt|collapse|ukraine.?war|russian.?invasion|руїн|разруш|обстріл|обстрел|scaffold|scaffolding|риштуван/i.test(
    u,
  );
}

/** Prefer inviting travel photos for main card / page 1. */
export function scoreAttractivePhotoUrl(url: string) {
  const u = String(url || '').toLowerCase();
  if (!u) return -999;
  let s = 0;
  if (isJunkLandmarkPhotoUrl(u)) s -= 200;
  ...
  if (/interior|museum.?exhibit|plaque|sign|map|diagram|plan|drawing|engraving|postcard|bw_|black.?white|sepia/i.test(u))
    s -= 25;
  if (/1280px|1920px|2048px/i.test(u)) s += 5;
  return s;
}

export function rankImagesForHero(urls: string[]) {
  return [...new Set((urls || []).filter(Boolean))].sort(
    (a, b) => scoreAttractivePhotoUrl(b) - scoreAttractivePhotoUrl(a),
  );
}
...
async function fetchImageBytesForEdit(url: string): Promise<{ buf: Buffer; mime: string; name: string } | null> {
  ...
  try {
    ...
    if (buf.length < 80 || buf.length > 12 * 1024 * 1024) return null;
    const ct = String(res.headers.get('content-type') || '').toLowerCase();
    const ext = extFromUrlOrType(src, ct);
    const mime =
      ct.includes('png') || ext === '.png'
        ? 'image/png'
  ...
  try {
    ...
    if (input.historicRefUri) {
      ...
      if (hist) {
        ...
        form.append(
          'image',
          new Blob([new Uint8Array(hist.buf)], { type: hist.mime }),
          `historic-${hist.name}`,
        );
      }
    }
    ...
    if (!res.ok) {
      ...
      if (res.status === 400 || res.status === 422) {
        ...
        form2.append(
          'image',
          new Blob([new Uint8Array(modern.buf)], { type: modern.mime }),
          modern.name,
        );
 ...
 */
export function scoreHistoricPhotoUrl(url: string) {
  const u = String(url || '').toLowerCase();
  if (!u) return -999;
  let s = 0;
  if (/postcard|engraving|lithograph|etching|drawing|painting|archive|antique|vintage|sepia|black.?white|bw_|історич|старий|листівк|доревол/i.test(u))
    s += 50;
  if (/historic|history|old.?photo|19th|18th|1910|1900|1890|1880|1870|1920|1930|1940/i.test(u)) s += 35;
  if (/\b(1[6-8]\d{2}|19[0-4]\d)\b/.test(u)) s += 40;
  if (/modern|aerial|drone|202\d|201\d|200\d|facade.?today|night.?view|iphone|samsung/i.test(u)) s -= 40;
  if (isUnattractiveLandmarkPhotoUrl(u)) s -= 30;
  return s;
}

export function pickBestHistoricPhoto(urls: string[], minScore = 25) {
  const ranked = [...new Set((urls || []).filter(Boolean))].sort(
    (a, b) => scoreHistoricPhotoUrl(b) - scoreHistoricPhotoUrl(a),
  );
  const best = ranked[0] || '';
  if (!best || scoreHistoricPhotoUrl(best) < minScore) return '';
  return best;
}
...
async function persistBase64Image(b64: string, fileName = 'scene-ai.png'): Promise<string | null> {
...
async function generateLandmarkImageViaGemini(
  prompt: string,
  fileHint: string,
): Promise<string | null> {
  ...
  try {
    ...
    const b64 =
      (typeof pred?.bytesBase64Encoded === 'string' && pred.bytesBase64Encoded) ||
...
async function generateLandmarkImageViaOpenAI(
  prompt: string,
  fileHint: string,
): Promise<string | null> {
  const apiKey = aiRouteConfig.apiKey;
  ...
  const buildBody = (size: string): Record<string, unknown> => {
    ...
    if (isDalle) {
      ...
      if (model === 'dall-e-3') body.style = 'natural';
  ...
  try {
    const attempt = async (size: string) => {
      ...
      try {
        ...
        return { ok: false as const, status: res.status, raw: raw.slice(0, 240) };
      }
...
export async function generateLandmarkSceneImage(input: {
  ...
  for (const provider of order) {
    if (provider === 'gemini') {
      if (!geminiConfig.apiKey) continue;
      const url = await generateLandmarkImageViaGemini(prompt, fileHint);
      if (url) return url;
      continue;
    }
    if (provider === 'openai') {
      if (!aiRouteConfig.apiKey) continue;
  ...
  console.warn(
    '[landmarkImage] all providers failed — set GEMINI_API_KEY (Imagen) and/or OPENAI_API_KEY (gpt-image-1)',
  );
  return null;
}
...
export async function generateLandmarkHeroImage(input: {
  ...
  return generateLandmarkSceneImage({
    ...input,
    scene:
      'Travel magazine cover: beautiful sunny or golden-hour exterior so tourists want to visit. At most tiny distant people.',
...
export async function generateLandmarkStoryPageImages(input: {
...
}): Promise<{ hero: string; page1: string; pages: string[] }> {
  ...
  const mark = async (label: string, uri: string) => {
    done += 1;
    if (uri) okCount += 1;
  ...
  const page1Raw =
    (await generateLandmarkSceneImage({
      ...input,
      scene:
        'Photorealistic travel photo of the landmark exterior, natural light, inviting for visitors. Real photograph look, NOT an icon, NOT a logo, NOT a drawing, NOT a religious painting, NOT a map. Different framing than a classic postcard hero — slightly closer or angled.',
  ...
  const pages: string[] = Array.from({ length: 12 }, () => '');
  if (genPages) {
    for (let i = 0; i < pageScenes.length; i += 1) {
      const scene = pageScenes[i];
      if (!scene) continue; // compare slot
      await sleep(400);
      const uri =
        (await generateLandmarkSceneImage({
          ...input,
  ...
  if (okCount === 0) {
    console.warn(
      '[landmarkImage] all generations failed — set GEMINI_API_KEY (Imagen 4) and/or OPENAI_API_KEY (gpt-image-1)',
    );
  } else {
...
async function persistGeneratedImage(json: any, fileName = 'hero-ai.png'): Promise<string | null> {
...
export async function rehostRemoteImage(url: string): Promise<string | null> {
  const src = String(url || '').trim();
  if (!/^https?:\/\//i.test(src)) return null;
  if (isAlreadyHostedLandmarkMedia(src)) return src;
...
export async function rehostImageList(urls: string[], limit = 12): Promise<string[]> {
  const out: string[] = [];
  for (const url of urls.slice(0, limit)) {
    const hosted = await rehostRemoteImage(url);
    if (hosted && !out.includes(hosted)) out.push(hosted);
    await sleep(80);
  }
  return out;
}

function rewriteUri(map: Map<string, string>, uri: unknown) {
  const u = typeof uri === 'string' ? uri.trim() : '';
  if (!u) return u;
  return map.get(u) || u;
}

export async function rehostLandmarkMediaFields(landmark: any): Promise<any> {
  if (!landmark || typeof landmark !== 'object') return landmark;
  const next = JSON.parse(JSON.stringify(landmark));
  const candidates = new Set<string>();
  const add = (u: unknown) => {
    const s = typeof u === 'string' ? u.trim() : '';
    if (/^https?:\/\//i.test(s)) candidates.add(s);
  };
  ...
  for (const url of candidates) {
    const hosted = await rehostRemoteImage(url);
    if (hosted) map.set(url, hosted);
    await sleep(60);
  }

  if (next.thumbUri) next.thumbUri = rewriteUri(map, next.thumbUri);
  if (Array.isArray(next.galleryUris)) {
    next.galleryUris = next.galleryUris.map((u: string) => rewriteUri(map, u)).filter(Boolean);
  }
  if (story) {
    if (story.photoFact?.bgUri) story.photoFact.bgUri = rewriteUri(map, story.photoFact.bgUri);
    if (story.beforeAfter?.oldUri) story.beforeAfter.oldUri = rewriteUri(map, story.beforeAfter.oldUri);
    if (story.beforeAfter?.newUri) story.beforeAfter.newUri = rewriteUri(map, story.beforeAfter.newUri);
    if (story.introPage1PhotoUri) story.introPage1PhotoUri = rewriteUri(map, story.introPage1PhotoUri);
    for (const pagesKey of ['introPagesUk', 'introPagesEn']) {
      const pages = Array.isArray(story[pagesKey]) ? story[pagesKey] : [];
      for (const page of pages) {
        if (!page || typeof page !== 'object') continue;
        for (const key of ['photoUri', 'secondaryPhotoUri', 'compareBeforeUri', 'compareAfterUri', 'illustrationUri']) {
          if (page[key]) page[key] = rewriteUri(map, page[key]);
        }
      }
    }
    if (Array.isArray(story.personMentions)) {
      story.personMentions = story.personMentions.map((person: any) => {
