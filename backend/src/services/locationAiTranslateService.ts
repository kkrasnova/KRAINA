...
export function clampGuideParagraphs(text: string, max = 5) {
  ...
  const paras = raw
    ...
    .filter(Boolean);
  if (paras.length <= max) return paras.join('\n\n');
...
function endsIncompleteGuidePara(para: string) {
...
function startsContinuationGuidePara(para: string) {
  const t = String(para || '').trim();
  if (!t) return false;
...
function shouldMergeGuideParas(prev: string, next: string) {
  if (!prev || !next) return false;
  if (endsIncompleteGuidePara(prev)) return true;
...
export function normalizeGuideProse(text: string, max = 5): string {
  const raw = clean(text)
    .replace(/\r\n/g, '\n')
    .replace(/[ \t]+\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n');
 ...
 */
export function ensureMinGuideParagraphs(text: string, min = 3, max = 5) {
  ...
  if (cleaned.length <= 1) return blob;
...
function stripGuideSectionLead(text: string): string {
  let raw = clean(text)
  ...
  for (let i = 0; i < 2; i += 1) {
    ...
    const isLabel =
      GUIDE_SECTION_LABELS.has(first) ||
...
function fillFromFallback(map: LangMap, fallbackEn: string, fallbackUk: string): LangMap {
...
function chunkLangs(langs: string[], size: number) {
  const out: string[][] = [];
  for (let i = 0; i < langs.length; i += size) out.push(langs.slice(i, i + size));
  return out;
}

function mergeLangMaps(base: LangMap, extra: LangMap | undefined | null): LangMap {
...
type GuideTranslateSource = {
  title: { uk: string; en: string };
  desc: { uk: string; en: string };
  shortIntro: { uk: string; en: string };
  miniPreview: { uk: string; en: string };
  introPage1: { uk: string; en: string };
  pages: Array<{ uk: string; en: string }>;
  quiz: {
    question: { uk: string; en: string };
    hint: { uk: string; en: string };
    options: Array<{ text: { uk: string; en: string }; correct: boolean }>;
    questions?: Array<{
      question: { uk: string; en: string };
      hint: { uk: string; en: string };
      options: Array<{ text: { uk: string; en: string }; correct: boolean }>;
  ...
  photoFact: { title: { uk: string; en: string }; body: { uk: string; en: string } };
  secondFact: { title: { uk: string; en: string }; body: { uk: string; en: string } };
};

type GuideTranslateResult = {
  title: LangMap;
  desc: LangMap;
  shortIntro: LangMap;
  miniPreview: LangMap;
  ...
  quiz: {
    question: LangMap;
    hint: LangMap;
    options: Array<{ text: LangMap; correct: boolean }>;
    questions?: Array<{
      question: LangMap;
      hint: LangMap;
      options: Array<{ text: LangMap; correct: boolean }>;
  ...
  photoFact: { title: LangMap; body: LangMap };
  secondFact: { title: LangMap; body: LangMap };
};

function emptyGuideResult(pageCount: number, optionCount: number): GuideTranslateResult {
  return {
    title: {},
    desc: {},
    shortIntro: {},
...
function qualityTranslateSystemPrompt(langs: string[]) {
...
async function openaiTranslateGuideBatch(
  source: GuideTranslateSource,
  langs: string[],
): Promise<Partial<GuideTranslateResult> | null> {
  const apiKey = aiRouteConfig.apiKey;
  if (!apiKey || !langs.length) return null;
  ...
  try {
    ...
    return parsed as Partial<GuideTranslateResult>;
  } catch (e) {
...
function asLangMap(v: unknown): LangMap {
 ...
 */
export async function localizeLandmarkForAllAppLanguages(
  landmark: any,
  opts?: {
  ...
  const quizQuestionsRaw = Array.isArray((quiz as any).questions) ? (quiz as any).questions : [];
  ...
  const mapQuizQuestionSource = (q: any) => ({
    question: {
  ...
  const source: GuideTranslateSource = {
  ...
  for (let bi = 0; bi < batches.length; bi += 1) {
    ...
    if (Array.isArray(part.pages)) {
      part.pages.forEach((pageMap, i) => {
        if (!merged.pages[i]) merged.pages[i] = {};
        merged.pages[i] = mergeLangMaps(merged.pages[i], asLangMap(pageMap));
      });
    }
    if (part.quiz && typeof part.quiz === 'object') {
      merged.quiz.question = mergeLangMaps(merged.quiz.question, asLangMap((part.quiz as any).question));
      merged.quiz.hint = mergeLangMaps(merged.quiz.hint, asLangMap((part.quiz as any).hint));
      const optsQuiz = Array.isArray((part.quiz as any).options) ? (part.quiz as any).options : [];
      optsQuiz.forEach((o: any, i: number) => {
        ...
        if (typeof o?.correct === 'boolean') merged.quiz.options[i].correct = o.correct;
      });
      const partQuestions = Array.isArray((part.quiz as any).questions) ? (part.quiz as any).questions : [];
      partQuestions.forEach((pq: any, qi: number) => {
        if (!merged.quiz.questions) merged.quiz.questions = [];
        ...
        pqOpts.forEach((o: any, oi: number) => {
          if (!mq.options[oi]) {
            mq.options[oi] = {
              text: {},
          ...
          if (typeof o?.correct === 'boolean') mq.options[oi].correct = o.correct;
        });
      });
    }
    if (part.photoFact && typeof part.photoFact === 'object') {
      merged.photoFact.title = mergeLangMaps(merged.photoFact.title, asLangMap((part.photoFact as any).title));
      merged.photoFact.body = mergeLangMaps(merged.photoFact.body, asLangMap((part.photoFact as any).body));
    }
    if (part.secondFact && typeof part.secondFact === 'object') {
      merged.secondFact.title = mergeLangMaps(
        merged.secondFact.title,
        asLangMap((part.secondFact as any).title),
      );
      merged.secondFact.body = mergeLangMaps(merged.secondFact.body, asLangMap((part.secondFact as any).body));
    }
  }
  ...
  const introPage1I18n = fillFromFallback(
    merged.introPage1,
    introPage1En || introPage1Uk,
    introPage1Uk || introPage1En,
  );
  const introPagesBodiesI18n = merged.pages.map((pageMap, i) =>
    fillFromFallback(pageMap, pagePairs[i]?.en || '', pagePairs[i]?.uk || ''),
  );
  ...
  const stripParen = (s: string) =>
    String(s || '')
      ...
      .trim();
  for (const lang of LANDMARK_CONTENT_LANGS) {
    if (titleI18n[lang]) titleI18n[lang] = stripParen(titleI18n[lang]) || titleI18n[lang];
  }
  ...
  const quizNext = {
    ...
    questionI18n: fillFromFallback(
      merged.quiz.question,
      source.quiz.question.en,
      source.quiz.question.uk,
    ),
    multiHintI18n: fillFromFallback(merged.quiz.hint, source.quiz.hint.en, source.quiz.hint.uk),
    options: quizOptions.map((o: any, i: number) => {
      const textMap = fillFromFallback(
        merged.quiz.options[i]?.text || {},
        source.quiz.options[i]?.text.en || '',
        source.quiz.options[i]?.text.uk || '',
      );
      return {
        ...o,
    ...
    questions: (source.quiz.questions || []).map((srcQ, qi) => {
      ...
      const questionI18n = fillFromFallback(
        mq?.question || {},
        srcQ.question.en,
        srcQ.question.uk,
      );
  ...
  next.story = {
    ...
    photoFact: {
      ...
      titleI18n: fillFromFallback(
        merged.photoFact.title,
        source.photoFact.title.en,
        source.photoFact.title.uk,
      ),
      bodyI18n: fillFromFallback(
        merged.photoFact.body,
        source.photoFact.body.en,
        source.photoFact.body.uk,
      ),
    },
    secondFact: {
      ...secondFact,
      ...
      titleI18n: fillFromFallback(
        merged.secondFact.title,
        source.secondFact.title.en,
        source.secondFact.title.uk,
      ),
      bodyI18n: fillFromFallback(
        merged.secondFact.body,
        source.secondFact.body.en,
        source.secondFact.body.uk,
      ),
    },
  };

  const translatedLangs = LANDMARK_CONTENT_LANGS.filter(
    (l) => l !== 'uk' && l !== 'en' && clean(titleI18n[l]) && clean(titleI18n[l]) !== titleEn,
  );
  console.info(
    `[landmarkTranslate] localized «${next.titleEn || next.titleUk}»: ${translatedLangs.length + 2}/${LANDMARK_CONTENT_LANGS.length} langs, ${introPagesBodiesI18n.length} pages`,
  );
...
function parseLandmarkGuideJson(
  text: string,
  input: {
    titleUk: string;
    titleEn: string;
  },
): LandmarkGuidePack | null {
  const parsed = parseJsonObject(text);
  if (!parsed) return null;

    const pagesUk = Array.isArray(parsed?.pagesUk)
      ? parsed.pagesUk
          .map((x: unknown) => ensureMinGuideParagraphs(stripGuideSectionLead(clean(x)), 5, 6))
          .filter(Boolean)
      : [];
    const pagesEn = Array.isArray(parsed?.pagesEn)
      ? parsed.pagesEn
          ...
          .filter(Boolean)
      : [];
  while (pagesUk.length < 12) pagesUk.push('');
  while (pagesEn.length < 12) pagesEn.push('');
  const quizRaw = parsed?.quiz && typeof parsed.quiz === 'object' ? parsed.quiz : null;

  const normalizeQuizOptions = (rawOptions: unknown): LandmarkGuideQuizOption[] => {
    ...
    const correctCount = options.filter((o: LandmarkGuideQuizOption) => o.correct).length;
  ...
  const normalizeOneQuizQuestion = (rawQ: any): LandmarkGuideQuizQuestion | null => {
  ...
  const questionsFromAi: LandmarkGuideQuizQuestion[] = [];
  if (Array.isArray(quizRaw?.questions)) {
    for (const q of quizRaw.questions) {
      const n = normalizeOneQuizQuestion(q);
      if (n) questionsFromAi.push(n);
      if (questionsFromAi.length >= 3) break;
    }
  }
  if (!questionsFromAi.length) {
    const legacy = normalizeOneQuizQuestion(quizRaw);
    if (legacy) questionsFromAi.push(legacy);
  }

  const PLACE_OR_NON_PERSON_RE =
    /\b(вул(?:иця|иці|ицею|ицю)?|просп(?:ект|екті|екту)?|площа|площі|майдан|набережн\w*|перевулок|alley|street|st\.|ave\.|avenue|road|square|plaza|park|парк|сад|монастир|церкв\w*|костел|собор|храм|каплиц\w*|castle|palace|museum|музей|міст|bridge|район|область|країна|city|місто|город|київ|kyiv)\b/i;
  const isLikelyPerson = (n: string) => {
    const t = clean(n);
    if (t.length < 5 || t.length > 72) return false;
    if (PLACE_OR_NON_PERSON_RE.test(t)) return false;
    const words = t.split(/\s+/).filter(Boolean);
  ...
  const people = peopleRaw
    ...
    .filter(
      (p: { nameUk: string; nameEn: string }) =>
        isLikelyPerson(p.nameUk) || isLikelyPerson(p.nameEn),
    )
    .slice(0, 12);
  const boldRe = /\*\*([^*]{4,64})\*\*/g;
  ...
  while ((bm = boldRe.exec(harvest)) !== null) {
    const n = clean(bm[1]);
    if (!isLikelyPerson(n)) continue;
    if (!people.some((p: { nameUk: string; nameEn: string }) => p.nameUk === n || p.nameEn === n)) {
