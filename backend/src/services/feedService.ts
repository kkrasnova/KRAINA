...
export async function saveFeedMediaFile(file: {
  buffer: Buffer;
  mimetype: string;
}): Promise<{ url: string; media_kind: 'image' | 'video' }> {
  ...
  const media_kind: 'image' | 'video' = file.mimetype.startsWith('video/') ? 'video' : 'image';
...
// Add: before_created_at?: string, before_id?: string params.
// WHERE addition: AND (p.created_at, p.id) < ($beforeCreatedAt, $beforeId)
export async function listUserPostsForViewer(viewerId: string, targetUsername: string, limit = 40) {
  ...
  const followR = await pool.query(
  ...
  const r = await pool.query(
    `SELECT p.*, pr.username, pr.avatar_url,
     ...
     ORDER BY p.created_at DESC
     LIMIT $2`,
    isOwner ? [ownerId, lim, viewerId] : [ownerId, lim, viewerId, canSeeFollowersScope],
  );
  return r.rows.map((row) =>
    mapPostRow(row as Record<string, unknown>, {
      username: String(row.username),
...
export async function createStory(userId: string, media_url: string, media_kind: 'image' | 'video', caption: string) {
