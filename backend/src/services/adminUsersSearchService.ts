...
export async function searchUsersByEmailFragment(q: string, limit = 25): Promise<AdminUserSearchRow[]> {
  const term = q
    .trim()
    .slice(0, 200)
    .replace(/\\/g, '')
  ...
  const r = await pool.query(
    `WITH matched_by_email AS (
     ...
     ORDER BY
       ...
       u.email ASC
     LIMIT $4`,
    [`%${term}%`, term, candidateLimit, lim],
  );
  return r.rows as AdminUserSearchRow[];
}
