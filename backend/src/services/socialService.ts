...
async function resolveFollowerPostgresIdForEdge(rawFollowerId: string): Promise<string | null> {
  ...
  return String((r2.rows[0] as { id: string }).id);
}
...
export async function resolveUserIdByUsername(username: string): Promise<string | null> {
  ...
  const r = await pool.query(
    `SELECT p.user_id::text AS id
     ...
     WHERE lower(p.username) = lower($1) AND u.status = 'active'`,
    [u],
  );
  if (!r.rowCount) return null;
  return String((r.rows[0] as { id: string }).id);
...
async function isProfilePrivate(userId: string): Promise<boolean> {
  const r = await pool.query(`SELECT is_public FROM profiles WHERE user_id = $1::uuid`, [userId]);
  if (!r.rowCount) return false;
  return (r.rows[0] as { is_public: boolean }).is_public === false;
}
...
async function _isMutualFollowWithClient(client: PoolClient, a: string, b: string): Promise<boolean> {
  if (a === b) return false;
...
async function _clearPendingIfMutualWithClient(client: PoolClient, userA: string, userB: string): Promise<void> {
  ...
  if (!mutual) return;
  await client.query(
    `UPDATE dm_threads SET pending_for_user_id = NULL, updated_at = now()
     WHERE user_low = $1::uuid AND user_high = $2::uuid`,
    [low, high],
  );
}
...
async function _insertConfirmedFollowEdgeWithClient(
  client: PoolClient,
  followerId: string,
  followingId: string,
): Promise<boolean> {
  if (followerId === followingId) throw new HttpError(400, 'cannot_follow_self');
  ...
  if (!ins.rowCount) return false; // edge already existed — counters are unchanged

  await client.query(
    `UPDATE profiles SET followers_count = followers_count + 1, updated_at = now() WHERE user_id = $1::uuid`,
    [followingId],
  );
  await client.query(
    `UPDATE profiles SET following_count = following_count + 1, updated_at = now() WHERE user_id = $1::uuid`,
    [followerId],
  );
  ...
  await client.query(
    `DELETE FROM follow_requests WHERE follower_id = $1::uuid AND followee_id = $2::uuid`,
    [followerId, followingId],
  );
...
async function insertConfirmedFollowEdge(followerId: string, followingId: string): Promise<boolean> {
  if (followerId === followingId) throw new HttpError(400, 'cannot_follow_self');
...
async function insertPendingFollowRequest(followerId: string, followeeId: string): Promise<boolean> {
  ...
  const ins = await pool.query(
    `INSERT INTO follow_requests (follower_id, followee_id) VALUES ($1::uuid, $2::uuid)
     ON CONFLICT DO NOTHING
     RETURNING follower_id`,
    [followerId, followeeId],
  );
...
async function deletePendingFollowRequest(followerId: string, followeeId: string): Promise<boolean> {
...
export async function insertFollowEdgeByIds(
  followerId: string,
  followingId: string,
...
export async function deleteFollowEdgeByIds(
  followerId: string,
  followingId: string,
): Promise<void> {
  ...
  try {
    ...
    const del = await client.query(
      `DELETE FROM follows WHERE follower_id = $1::uuid AND following_id = $2::uuid RETURNING follower_id`,
      [followerId, followingId],
    );
    if (!del.rowCount) {
    ...
    await client.query(
      `UPDATE profiles SET followers_count = GREATEST(0, followers_count - 1), updated_at = now() WHERE user_id = $1::uuid`,
      [followingId],
    );
    await client.query(
      `UPDATE profiles SET following_count = GREATEST(0, following_count - 1), updated_at = now() WHERE user_id = $1::uuid`,
      [followerId],
    );
    await client.query('COMMIT');
  } catch (e) {
...
export async function unfollowByUsername(followerId: string, targetUsername: string): Promise<void> {
  const targetId = await resolveUserIdByUsername(targetUsername);
  if (!targetId) throw new HttpError(404, 'user_not_found');
  if (targetId === followerId) throw new HttpError(400, 'cannot_unfollow_self');
  await deleteFollowEdgeByIds(followerId, targetId);
}

export async function unfollowByUserId(followerId: string, targetUserId: string): Promise<void> {
  const targetId = String(targetUserId || '').trim();
  if (!PG_UUID_RE.test(targetId)) throw new HttpError(400, 'invalid_user_id');
  if (targetId === followerId) throw new HttpError(400, 'cannot_unfollow_self');
  await deleteFollowEdgeByIds(followerId, targetId);
...
export async function listIncomingRequests(userId: string): Promise<FriendRequestRow[]> {
  const r = await pool.query(
    `SELECT u.id::text AS user_id, p.username, p.display_name, p.avatar_url, fr.requested_at
...
export async function listOutgoingRequests(userId: string): Promise<FriendRequestRow[]> {
  const r = await pool.query(
    `SELECT u.id::text AS user_id, p.username, p.display_name, p.avatar_url, fr.requested_at
     FROM follow_requests fr
...
export async function cancelOutgoingRequest(meId: string, targetUserId: string): Promise<void> {
...
export async function acceptFirestoreFollowIntoPostgres(followeeId: string, followerParam: string): Promise<void> {
  const fp = String(followerParam || '').trim();
  if (!followeeId || !fp) throw new HttpError(400, 'invalid_params');
  ...
  if (!followerPgId) {
    if (PG_UUID_RE.test(fp)) throw new HttpError(400, 'invalid_follower');
    throw new HttpError(400, 'follower_not_linked');
  }
  if (followerPgId === followeeId) throw new HttpError(400, 'cannot_follow_self');
  
  await deletePendingFollowRequest(followerPgId, followeeId);
  await insertConfirmedFollowEdge(followerPgId, followeeId);
}

export async function listMutualFriends(userId: string): Promise<MutualFriendRow[]> {
  const r = await pool.query(
    `SELECT u.id::text AS user_id, p.username, p.avatar_url
     FROM follows f1
     INNER JOIN follows f2
  ...
  return (r.rows as MutualFriendRow[]).map((row) => ({
    user_id: String(row.user_id),
...
export async function searchSocialProfiles(meId: string | null, q: string, limit = 24): Promise<SocialSearchRow[]> {
  const term = String(q || '')
    .trim()
    .slice(0, 200)
    .replace(/\\/g, '')
...
export async function listSocialConnectionsByTarget(params: {
  ...
  const [rel, pend] = await Promise.all([
    pool.query(
      `SELECT following_id::text AS id
       FROM follows
  ...
  const followingSet = new Set((rel.rows as Array<{ id: string }>).map((x) => String(x.id)));
  const pendingSet = new Set((pend.rows as Array<{ id: string }>).map((x) => String(x.id)));
...
export async function getRelationStateByTarget(params: {
  ...
  const row = r.rows[0] as
    | { is_following: boolean; pending_follow_outgoing: boolean; is_followed_by_peer: boolean }
    | undefined;
