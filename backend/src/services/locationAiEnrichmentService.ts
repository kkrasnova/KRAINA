import {
  rehostLandmarkMediaFields,
  generateLandmarkStoryPageImages,
  generateLandmarkHistoricCompareImage,
  rankImagesForHero,
  isUnattractiveLandmarkPhotoUrl,
  isJunkLandmarkPhotoUrl,
  isNonPhotoJunkUrl,
  isDocumentaryDamagePhotoUrl,
  pickBestHistoricPhoto,
  scoreHistoricPhotoUrl,
} from './locationAiImageRehostService.js';
import {
  localizeLandmarkForAllAppLanguages,
  openaiBuildLandmarkGuide,
  ensureMinGuideParagraphs,
  clampGuideParagraphs,
  normalizeGuideProse,
  splitGuideSentences,
} from './locationAiTranslateService.js';
...
const COMPARE_PAGE3_LAYOUT: Record<string, unknown> = {
 ...
 */
const INTRO_PAGE_LAYOUTS: Record<string, unknown>[] = [
  {
    heroFit: 'cover',
    heroHeightRatio: 0.4,
    heroHeightMax: 340,
...
export type EnrichItemTrace = {
  ...
  queries: string[];
  wikiEn: { title: string; url: string; candidates: string[] } | null;
  wikiUk: { title: string; url: string; candidates: string[] } | null;
...
type WikiHit = {
  origin: string;
  title: string;
  data?: { extract: string; pageTitle: string; missing?: boolean; lat: number | null; lng: number | null };
  candidates?: string[];
};
...
async function withTimeout<T>(promise: Promise<T>, ms: number, fallback: T, label = 'step'): Promise<T> {
...
function previewText(s: unknown, max = 420) {
  const t = String(s || '')
    .replace(/\s+/g, ' ')
    .trim();
...
function normalizeWikiTitle(s: unknown) {
  return String(s || '')
    .toLowerCase()
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
...
const PLACE_TYPE_GROUPS: Array<{ id: string; keys: string[]; anti: string[] }> = [
  {
    id: 'church',
    keys: [
      'костел',
...
function typeMismatchPenalty(queryName: string, pageTitle: string): number {
  ...
  for (const id of qTypes) {
    ...
    if (group.anti.some((a) => titleNorm.includes(a))) penalty -= 200;
...
function nameTitleCoverage(nameWords: string[], titleNorm: string): number {
...
function expandPlaceTokens(city: string, country: string): string[] {
...
function cityTitleForms(city: string, placeTokens: string[]): string[] {
  const preferred: string[] = [];
  const push = (s: string) => {
    const t = String(s || '').trim();
    if (!t) return;
    if (!preferred.some((x) => normalizeWikiTitle(x) === normalizeWikiTitle(t))) preferred.push(t);
  };
  push(city);
  ...
  for (const form of canon[key] || []) push(form);
  for (const t of placeTokens) {
    if (canon[t]) for (const form of canon[t]) push(form);
  }
  // Capitalize alias tokens as a last resort
  for (const t of placeTokens.slice(0, 4)) {
    if (t.length >= 3) push(t.charAt(0).toUpperCase() + t.slice(1));
  }
...
function isBadWikiPage(title: string, extractOrSnippet = ''): boolean {
  const t = String(title || '');
  const body = String(extractOrSnippet || '');
  if (/значення|disambiguation|may refer to|^список |list of |категорія:|category:/i.test(t)) return true;
  if (/може означати|може відноситись|may refer to|disambiguation page/i.test(body.slice(0, 500))) return true;
  // list-like: many other cities enumerated without focus
  const bullets = (body.match(/\n\*\s+/g) || []).length;
  if (bullets >= 8 && body.length < 2500) return true;
  return false;
}
...
function scorePlaceMatch(input: {
  ...
  if (isBadWikiPage(input.title, `${input.snippet || ''} ${input.extract || ''}`)) return -10000;
  ...
  const hasPlaceInTitleParen =
    !!parenNorm && places.some((p) => parenNorm === p || parenNorm.includes(p) || p.includes(parenNorm));
  const hasPlaceInTitle = places.some((p) => title.includes(p));
  const hasPlaceInBody = places.some((p) => body.includes(p));

  if (hasPlaceInTitleParen) score += 80;
  else if (hasPlaceInTitle) score += 50;
  if (hasPlaceInBody) score += 25;
  // require city somehow when places provided
  if (places.length && !hasPlaceInTitle && !hasPlaceInBody) score -= 80;

  for (const a of input.addressTokens) {
    if (title.includes(a)) score += 70;
    else if (body.includes(a)) score += 45;
  }

  const coverage = nameTitleCoverage(input.nameWords, title);
  let nameHits = 0;
  for (const w of input.nameWords) {
    if (title.includes(w)) nameHits += 1;
  }
  ...
  let bodyOnlyHits = 0;
  for (const w of input.nameWords) {
    if (!title.includes(w) && body.includes(w)) bodyOnlyHits += 1;
  }
  ...
  if (/cultural.*(heritage|sites)|heritage sites|пам.ятки.*війни|втрачен/i.test(input.title)) score -= 60;

  return score;
}

async function wikiExtract(origin: string, title: string) {
  const u = `${origin}/w/api.php?action=query&redirects=1&prop=extracts|coordinates&explaintext=1&titles=${encodeURIComponent(
    title,
  )}&format=json`;
  const json = await fetchJson(u);
  const page = Object.values(json?.query?.pages || {})[0] as any;
  if (!page || page.missing) return { extract: '', pageTitle: title, missing: true, lat: null, lng: null };
...
async function wikiLangLink(origin: string, title: string, targetLang: string): Promise<string | null> {
  const u = `${origin}/w/api.php?action=query&redirects=1&prop=langlinks&lllang=${encodeURIComponent(
    targetLang,
...
async function wikiSearchCandidates(
  ...
  const u = `${origin}/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(
    query,
...
async function resolveWikiHitForPlace(input: {
  ...
  const candidateTitles = new Map<string, string>(); // title -> snippet
  const addCand = (title: string, snippet = '') => {
    if (!title || isBadWikiPage(title, snippet)) return;
    if (typeMismatchPenalty(input.name, title) <= -150) return;
    if (!candidateTitles.has(title)) candidateTitles.set(title, snippet);
  };
  ...
  const acceptHit = (pageTitle: string, extract: string, score: number) => {
    const cov = nameTitleCoverage(nameWords, normalizeWikiTitle(pageTitle));
    if (typeMismatchPenalty(input.name, pageTitle) <= -150) return false;
    if (nameWords.length >= 1 && cov < 0.34) return false;
    ...
    if (cov >= 0.6 && (hasCity || hasAddr) && score >= 40) return true;
    if (cov >= 0.34 && hasCity && score >= 50) return true;
  ...
  for (const q of queries) {
    ...
    for (const h of hits) addCand(h.title, h.snippet);
    await sleep(80);
  }

  const ranked: Array<{ title: string; snippet: string; score: number }> = [];
  ...
  let best: WikiHit | null = null;
  let bestScore = -Infinity;
  for (const row of ranked.slice(0, 8)) {
    const data = await wikiExtract(origin, row.title);
    if (data.missing || !data.extract || data.extract.length < 80) continue;
    if (isBadWikiPage(data.pageTitle, data.extract)) continue;
    const score = scorePlaceMatch({
    ...
    if (!acceptHit(data.pageTitle, data.extract, score)) continue;
    if (score > bestScore) {
      bestScore = score;
      best = {
        origin,
...
async function wikidataSearchEntity(name: string, city: string, lang: 'uk' | 'en') {
  const q = `${name} ${city}`.trim();
  const u = `https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(
    q,
  )}&language=${lang}&uselang=${lang}&limit=6&format=json`;
  const json = await fetchJson(u, 2);
  ...
  for (const row of rows) {
    ...
    if (cov < 0.34 && nameWords.length) continue;
    if (city && !cityOk && cov < 0.8) continue;
    return { id, label, description: desc };
  }
  // retry bare name
  const u2 = `https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${encodeURIComponent(
    name,
  )}&language=${lang}&uselang=${lang}&limit=6&format=json`;
  ...
  for (const row of Array.isArray(json2?.search) ? json2.search : []) {
    ...
    if (cov >= 0.5 && (cityOk || cov >= 0.99)) return { id, label, description: desc };
  }
  return null;
}

async function wikidataEntityFacts(id: string, lang: 'uk' | 'en') {
  const u = `https://www.wikidata.org/wiki/Special:EntityData/${encodeURIComponent(id)}.json`;
  const json = await fetchJson(u, 2);
  const entity = json?.entities?.[id];
  if (!entity) return { text: '', lat: null as number | null, lng: null as number | null, url: '' };
  const labels = entity.labels || {};
  const descriptions = entity.descriptions || {};
  const label =
    labels[lang]?.value || labels.uk?.value || labels.en?.value || labels[Object.keys(labels)[0] || '']?.value || '';
  const description =
  ...
  const aliases = [
    ...
    ...((entity.aliases?.[lang] || []).map((a: any) => a?.value).filter(Boolean) || []),
  ];

  // Pull structured facts: inception, architects, significant events, destruction…
  const claimBits: string[] = [];
  const timeClaim = (pid: string, labelText: string) => {
    ...
    for (const row of rows.slice(0, 4)) {
      ...
      if (typeof t === 'string' && t.length >= 5) {
        ...
        if (year) claimBits.push(`${labelText}: ${year}`);
      }
    }
  };
  ...
  const idClaims = (pid: string, labelText: string) => {
    const rows = Array.isArray(entity.claims?.[pid]) ? entity.claims[pid] : [];
    for (const row of rows.slice(0, 6)) {
      const qid = row?.mainsnak?.datavalue?.value?.id;
      if (typeof qid === 'string' && qid) claimBits.push(`${labelText}: ${qid}`);
    }
  };
...
async function gatherDeepLandmarkContext(input: {
  ...
  extractUk: string;
  extractEn: string;
  emit: (e: Omit<EnrichLogEntry, 'ts'>) => Promise<void>;
  itemIndex: number;
}): Promise<{
  extractUk: string;
  extractEn: string;
  sources: string[];
  documentaryImages: string[];
}> {
  const { name, titleUk, titleEn, city, emit, itemIndex } = input;
  const sources: string[] = [];
  const documentaryImages: string[] = [];
  const extrasUk: string[] = [];
  ...
  const mentionsLandmark = (title: string, snippet: string, extract: string) => {
    const blob = normalizeWikiTitle(`${title} ${snippet} ${extract.slice(0, 800)}`);
    if (!blob) return false;
    if (nameKey && blob.includes(nameKey.slice(0, Math.min(18, nameKey.length)))) return true;
    if (titleUkKey && blob.includes(titleUkKey.slice(0, Math.min(18, titleUkKey.length)))) return true;
    if (titleEnKey && blob.includes(titleEnKey.slice(0, Math.min(18, titleEnKey.length)))) return true;
    // token overlap
  ...
  const harvestLang = async (lang: 'uk' | 'en', queries: string[]) => {
    const origin = `https://${lang}.wikipedia.org`;
    for (const q of queries) {
      ...
      for (const h of hits.slice(0, 3)) {
        ...
        if (isBadWikiPage(h.title, h.snippet)) continue;
        // Skip bare city / list-of-war-heritage dumps unless they mention the place
        if (
          /список|list of|втрачен|heritage of war|cultural property destroyed/i.test(h.title) &&
        ...
        await sleep(70);
        if (data.missing || !data.extract || data.extract.length < 120) continue;
        if (!mentionsLandmark(h.title, h.snippet, data.extract)) continue;
        seenTitles.add(key);
        const clipped = cleanWikiPlainText(data.extract).slice(0, 3500);
        if (lang === 'uk') extrasUk.push(`=== ${data.pageTitle} ===\n${clipped}`);
        else extrasEn.push(`=== ${data.pageTitle} ===\n${clipped}`);
        sources.push(pageUrl(origin, data.pageTitle));
        if (extrasUk.length + extrasEn.length >= 8) return;
      }
      await sleep(60);
    }
  };
  ...
  if (wd?.id) {
    ...
    if (factsUk.text) extrasUk.push(`=== Wikidata ===\n${factsUk.text}`);
    if (factsEn.text) extrasEn.push(`=== Wikidata ===\n${factsEn.text}`);
    if (factsUk.url) sources.push(factsUk.url);
  }
  ...
  for (const q of docQueries) {
    if (documentaryImages.length >= 10) break;
    const found = await commonsSearchImages(q, 6, { documentary: true });
    for (const url of found) {
      if (!documentaryImages.includes(url)) documentaryImages.push(url);
    }
    await sleep(100);
  }
...
function formatOsmStreetAddress(addr: any): string {
...
async function nominatimLookup(name: string, address: string | undefined, city: string, country: string) {
  const q = [name, address, city, country].filter(Boolean).join(', ');
  const u = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(
    q,
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (cityOk) s += 40;
      if (row?.type === 'museum' || row?.class === 'tourism' || row?.class === 'historic') s += 20;
      if (s > bestScore) {
        bestScore = s;
        best = row;
      }
...
async function gatherAltPlaceContext(input: {
  ...
  if (wdId) {
    ...
    if (factsUk.text) extractUk = factsUk.text;
    if (factsEn.text) extractEn = factsEn.text;
    if (factsUk.label) labelUk = factsUk.label;
    if (factsEn.label) labelEn = factsEn.label;
  ...
  if (osm) {
    if (!streetAddress && osm.streetAddress) streetAddress = osm.streetAddress;
    sources.push(osm.url);
    if (!extractUk) extractUk = osm.text;
    else extractUk = `${extractUk}\n\n${osm.text}`;
    if (!extractEn) extractEn = osm.text;
    else extractEn = `${extractEn}\n\n${osm.text}`;
    if (lat == null) lat = osm.lat;
    if (lng == null) lng = osm.lng;
    if (osm.wikidataId && !wdId) {
      ...
      if (factsEn.text) extractEn = `${factsEn.text}\n\n${extractEn}`;
      if (factsUk.url) sources.push(factsUk.url);
      if (lat == null) lat = factsUk.lat ?? factsEn.lat;
      if (lng == null) lng = factsUk.lng ?? factsEn.lng;
    }
    ...
    if (osm.wikipediaTitle) {
      ...
      const origin =
        wikiLang === 'en' ? 'https://en.wikipedia.org' : `https://${wikiLang || 'uk'}.wikipedia.org`;
      const data = await wikiExtract(origin, wikiTitle);
      if (!data.missing && data.extract.length > 80) {
        if (wikiLang === 'en') extractEn = data.extract;
        else extractUk = data.extract;
        sources.unshift(pageUrl(origin, data.pageTitle));
        if (lat == null) lat = data.lat;
        if (lng == null) lng = data.lng;
        await emit({
          level: 'ok',
          step: 'wiki',
          itemIndex,
...
async function wikiSummaryImage(origin: string, title: string) {
...
function personNameQueryVariants(name: string): string[] {
...
async function wikiPersonSummary(name: string, lang: 'uk' | 'en' | 'pl') {
  const origin = `https://${lang}.wikipedia.org`;
  ...
  for (const q of personNameQueryVariants(name).slice(0, 2)) {
    ...
    if (!title && hits[0]?.title) title = hits[0].title;
    if (!title) continue;
    const slug = encodeURIComponent(title.replace(/ /g, '_'));
    ...
    if (
      /street|avenue|square|church|cathedral|building|вулиця|площа|костел|церква|храм|київ|kyiv/i.test(
        String(sum?.title || title) + ' ' + desc,
      )
    ) {
      continue;
    }
    if (!titleLooksPerson) continue;
    return {
      title: String(sum?.title || title).trim(),
      photoUri: String(photo),
      wikiUrl: String(
        sum?.content_urls?.desktop?.page || `${origin}/wiki/${encodeURIComponent(title.replace(/ /g, '_'))}`,
      ),
      blurb: extract,
    };
  }
...
async function resolvePersonMentions(
  people: Array<{ nameUk: string; nameEn: string }> | undefined | null,
  emit?: (e: Omit<EnrichLogEntry, 'ts'>) => Promise<void>,
  itemIndex?: number,
  itemName?: string,
): Promise<PersonMentionResolved[]> {
  const list = Array.isArray(people) ? people : [];
  const out: PersonMentionResolved[] = [];
  ...
  for (const p of list.slice(0, 6)) {
    ...
    if (seen.has(key) || (!nameUk && !nameEn)) continue;
    const placeRe =
      /\b(вул|просп|площа|майдан|street|avenue|square|park|церкв|костел|собор|музей|київ|kyiv)\b/i;
    if (placeRe.test(nameUk) || placeRe.test(nameEn)) continue;
    const wordsUk = nameUk.split(/\s+/).filter(Boolean);
    const wordsEn = nameEn.split(/\s+/).filter(Boolean);
    if (wordsUk.length < 2 && wordsEn.length < 2) continue;
    seen.add(key);
    await emit?.({
      level: 'info',
      step: 'images',
    ...
    let hit: Awaited<ReturnType<typeof wikiPersonSummary>> = null;
    try {
      hit =
        (nameUk
...
function ensurePersonNamesBold(text: string, people: Array<{ nameUk: string; nameEn: string }>) {
  let out = String(text || '');
...
async function wikiPageImages(origin: string, title: string, limit = 10) {
  ...
  const files = (page?.images || [])
    .map((x: any) => x?.title)
    .filter((t: string) => t && !/icon|logo|flag|map|locator|symbol|coat|emblem/i.test(t));
  ...
  const urls: string[] = [];
  for (const p of Object.values(infoJson?.query?.pages || {}) as any[]) {
...
async function commonsSearchImages(
  query: string,
  limit = 10,
  opts?: { documentary?: boolean },
) {
  const u = `https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(
    query,
  )}&gsrnamespace=6&gsrlimit=${Math.max(limit, 12)}&prop=imageinfo&iiprop=url|extmetadata&iiurlwidth=1200&format=json`;
  const json = await fetchJson(u, 2);
  const urls: string[] = [];
  for (const p of Object.values(json?.query?.pages || {}) as any[]) {
    ...
    const url = p?.imageinfo?.[0]?.thumburl || p?.imageinfo?.[0]?.url;
    if (!url || !/^https?:\/\//i.test(url) || /\.svg$/i.test(url)) continue;
    if (opts?.documentary) {
      // Allow war/damage/scaffold for documentary pages — still reject icons/maps
      if (isNonPhotoJunkUrl(url)) continue;
...
function inferVisitCategoryFromTexts(...parts: Array<string | undefined | null>): 'monument' | 'museum' | 'park' | 'other' {
  const blob = parts.filter(Boolean).join(' ').toLowerCase();
  if (!blob.trim()) return 'monument';
  if (/(музей|museum|галерея|gallery|pinacoteca)/i.test(blob)) return 'museum';
  if (/(парк|park|сад|garden|ботаніч|botanic)/i.test(blob)) return 'park';
  if (
    /(церкв|костел|собор|храм|каплиц|монастир|лавра|church|cathedral|chapel|basilica|abbey|monastery|temple|mosque|синагог|замок|castle|palace|фортец|форт|пам.?ят|monument|statue|tower|вежа|міст|bridge)/i.test(
      blob,
    )
  ) {
    return 'monument';
  ...
  if (types.includes('church') || types.includes('monument') || types.includes('square')) return 'monument';
  return 'monument';
}

function cleanWikiPlainText(raw: string) {
  return normalizeGuideProse(
    String(raw || '')
...
function stripCityFromTitle(title: string, fallback = '') {
...
function uniqueSequentialChunks(paragraphs: string[], count: number) {
  const chunks = Array.from({ length: count }, () => '');
  const pool = paragraphs.filter(Boolean);
  for (let i = 0; i < count; i += 1) {
    if (i < pool.length) chunks[i] = pool[i];
  }
...
function buildBody(_title: string, _label: string, text: string) {
  // Never prepend section titles like "Історичний контекст." — start with the story.
  return cleanWikiPlainText(text || '');
}

function stripSlideLabelLead(text: string): string {
  const labels = new Set(
    [...SLIDE_LABELS_UK, ...SLIDE_LABELS_EN, 'деталі', 'details'].map((s) =>
      s.toLowerCase(),
    ),
  );
  ...
  for (let i = 0; i < 2; i += 1) {
    ...
    if (!isLabel || paras.length < 2) {
      if (isLabel && paras.length === 1) return '';
      break;
    }
...
function pageUrl(origin: string, title: string) {
...
async function collectImages(
  ...
  if (enHit) for (const url of await wikiPageImages(enHit.origin, enTitle)) push(url, 'EN page images');
  if (ukHit) for (const url of await wikiPageImages(ukHit.origin, ukTitle)) push(url, 'UK page images');
  const queries = [
  ...
  for (const q of queries) {
    ...
    if (images.length >= 12 && emptyStreak >= 2) break;
    ...
    if (!found.length) {
      emptyStreak += 1;
      if (emptyStreak >= 3 && images.length >= 8) break;
      continue;
    }
    emptyStreak = 0;
    for (const url of found) push(url, 'Commons');
    await sleep(120);
  }
...
async function collectHistoricImages(
  item: ImportItem,
  city: string,
  enTitle: string,
  ukTitle: string,
  ...
  const push = (url: string) => {
    if (url && !out.includes(url) && scoreHistoricPhotoUrl(url) >= 10) out.push(url);
  };
  const queries = [
...
async function resolvePage3ComparePair(input: {
  ...
  itemIndex: number;
  itemName: string;
}): Promise<{ oldUri: string; newUri: string }> {
  const newUri =
    input.modernUri ||
  ...
  const historicRefUri =
    historicRef && newUri && historicRef !== newUri ? historicRef : pickBestHistoricPhoto(
      historicPool.filter((u) => u && u !== newUri),
      20,
    );
  ...
  const aiOld = await withTimeout(
    generateLandmarkHistoricCompareImage({
      ...
      modernViewHint:
        'Match the modern Wikipedia/Commons photo viewpoint EXACTLY: same facade angle, towers, crop. Vertical 9:16 phone frame for a before/after slider.',
    }),
    120000,
    null,
    'historic-compare-ai',
  ...
  const weak = pickBestHistoricPhoto(
    historicPool.filter((u) => u && u !== newUri),
    5,
  );
...
function assignUniquePhotos(images: string[], need: number, used?: Set<string>) {
  ...
  if (!pool.length) {
    ...
    if (!raw.length) return Array.from({ length: need }, () => '');
...
function takeNextUniquePhoto(
  preferred: string | undefined | null,
  pools: string[][],
  used: Set<string>,
): string {
  const tryOne = (raw: unknown) => {
    const s = typeof raw === 'string' ? raw.trim() : '';
    if (!s || used.has(s) || isJunkLandmarkPhotoUrl(s)) return '';
    used.add(s);
    return s;
  };
...
function uniquePhotoList(urls: Array<string | undefined | null>) {
  const out: string[] = [];
  const seen = new Set<string>();
  for (const raw of urls) {
    const s = typeof raw === 'string' ? raw.trim() : '';
    if (!s || seen.has(s) || isJunkLandmarkPhotoUrl(s)) continue;
    seen.add(s);
    out.push(s);
  }
  return out;
...
function scoreHistoricUri(url: string) {
  ...
  if (/historic|history|old|vintage|postcard|engraving|archive|antique|sepia|black.?white|bw_|drawing|etching|lithograph|painting|історич|старий|доревол|листівк/i.test(u))
    s += 6;
  if (/\b(1[6-9]\d{2}|190\d|191\d|192\d|193\d|194\d)\b/.test(u)) s += 4;
  if (/modern|aerial|drone|202\d|201\d|facade|exterior|today|view_from/i.test(u)) s -= 3;
  return s;
}

function scoreModernUri(url: string) {
  const u = String(url || '').toLowerCase();
  let s = 0;
  if (/modern|aerial|drone|202\d|201\d|facade|exterior|panorama|view|today|night|sunset/i.test(u)) s += 5;
  if (/historic|old|vintage|postcard|engraving|archive|sepia|painting|історич|старий/i.test(u)) s -= 4;
  if (/\b(1[6-9]\d{2}|190\d|191\d|192\d)\b/.test(u)) s -= 3;
  return s;
}

/** Old (bottom) + new (top) for page-3 compare slider — two distinct photos. */
function pickBeforeAfterPair(images: string[], used: Set<string>) {
  ...
  if (pool.length < 2) {
    ...
    if (oldUri) used.add(oldUri);
    if (newUri) used.add(newUri);
...
function isPlaceholderQuizOption(text: string) {
  return /^(варіант|вариант|option)\s*\d+$/i.test(String(text || '').trim());
}

function normalizeQuizQuestionPack(raw: any) {
  ...
  if (!multiHintUk && correctUk) multiHintUk = `Правильна відповідь: **${correctUk}**.`;
  if (!multiHintEn && correctEn) multiHintEn = `Correct answer: **${correctEn}**.`;
...
function extractBoldPeople(text: string) {
  const names: string[] = [];
  const re = /\*\*([^*]{4,64})\*\*/g;
...
function distractorYears(year: string) {
  const y = Number(year);
...
function fallbackQuizQuestions(
  titleUk: string,
  titleEn: string,
  storyUk: string,
  storyEn: string,
  ...
  const out: NonNullable<ReturnType<typeof normalizeQuizQuestionPack>>[] = [];
  const seenQ = new Set<string>();

  const push = (q: NonNullable<ReturnType<typeof normalizeQuizQuestionPack>> | null) => {
    if (!q) return;
    const fp = `${q.questionUk}|${q.questionEn}`.toLowerCase().slice(0, 80);
    if (seenQ.has(fp)) return;
    seenQ.add(fp);
    out.push(q);
  };
  ...
  const styleHit = blob.match(
    /\b(неоготик\w*|neo-?gothic|барок\w*|baroque|ренесанс\w*|renaissance|модерн\w*|art nouveau|готик\w*|gothic|класици\w*|classicis\w*)\b/i,
  );
  if (styleHit) {
...
function buildStory(input: {
  ...
  images: string[];
  sources: string[];
  guide?: Awaited<ReturnType<typeof openaiBuildLandmarkGuide>> | null;
  /** AI/beautiful cover — used for card + page 1 (must not be ruins/scaffold). */
  heroUri?: string;
  /** AI photo for welcome page (defaults to heroUri). */
  page1PhotoUri?: string;
  /** AI photo per intro page index 0..11 (skip compare index 1). */
  pagePhotos?: string[];
  /** Explicit page-3 compare pair (old bottom / new top). */
  compareOldUri?: string;
  compareNewUri?: string;
  /** Pre-resolved people with portrait photos. */
  personMentions?: PersonMentionResolved[];
}) {
  ...
  const boldPeople = (t: string) => ensurePersonNamesBold(t, people);

  const globalUsedKeys = new Set<string>();
  const densifyPage = (raw: string, pool: string[], minChars = 900) => {
    ...
    t = t
      ...
      .filter((p) => {
        ...
        if (
          [...globalUsedKeys].some(
            (u) => u === key || (u.length > 36 && (key.startsWith(u) || u.startsWith(key))),
          )
        ) {
          return false;
        }
  ...
  const page1Uk = boldPeople(
    densifyPage(
      cleanWikiPlainText(guide?.introPage1Uk || '') ||
        cleanWikiPlainText(parasUk.slice(0, 6).join('\n\n') || input.descUk || titleUk),
      [...parasUk, ...(guide?.pagesUk || []), input.descUk || ''],
      1000,
    ),
  );
  const page1En = boldPeople(
    densifyPage(
      cleanWikiPlainText(guide?.introPage1En || '') ||
        cleanWikiPlainText(parasEn.slice(0, 6).join('\n\n') || input.descEn || titleEn),
      [...parasEn, ...(guide?.pagesEn || []), input.descEn || input.descUk || ''],
      1000,
    ),
  );

  const bodiesUkRaw =
    guide?.pagesUk?.some(Boolean)
      ? guide.pagesUk.map((t) =>
          boldPeople(densifyPage(t, [...parasUk, input.descUk || '', page1Uk], 900)),
        )
      : uniqueSequentialChunks(parasUk.slice(1), 12).map((t, i) =>
          densifyPage(
            buildBody(titleUk, SLIDE_LABELS_UK[i] || 'деталі', t),
            [...parasUk, input.descUk || '', page1Uk],
            700,
          ),
        );
  const bodiesEnRaw =
    guide?.pagesEn?.some(Boolean)
      ? guide.pagesEn.map((t) =>
          boldPeople(densifyPage(t, [...parasEn, input.descEn || '', page1En], 900)),
        )
      : uniqueSequentialChunks(parasEn.slice(1), 12).map((t, i) =>
          densifyPage(
            buildBody(titleEn, SLIDE_LABELS_EN[i] || 'details', t),
            [...parasEn, input.descEn || '', page1En],
            700,
          ),
        );

  const hasRichAiGuide = !!(
    (guide?.pagesUk?.filter((t) => cleanWikiPlainText(t).length >= 280) || []).length >= 6
  );
  ...
  const bodiesUk = Array.from({ length: 12 }, (_, i) => {
    ...
    if (t && t !== page1Uk && t.length >= 320) return t;
    if (hasRichAiGuide) {
      const alt = parasUk[i + 1] || parasUk[i + 2] || '';
      const altClean = densifyPage(alt, [...parasUk, fallbackDescUk], 700);
      if (altClean && altClean !== page1Uk && altClean.length >= 120) return altClean;
      return densifyPage(
        buildBody(
          titleUk,
          SLIDE_LABELS_UK[i] || 'деталі',
          parasUk[Math.min(i + 1, Math.max(0, parasUk.length - 1))] || fallbackDescUk || titleUk,
        ),
        [...parasUk, fallbackDescUk],
        700,
      );
    }
    ...
    if (i === 1) {
      return densifyPage(
        ...
        [...parasUk, fallbackDescUk, page1Uk],
        700,
      );
    }
    return '';
  ...
  const bodiesEn = Array.from({ length: 12 }, (_, i) => {
    ...
    if (t && t !== page1En && t.length >= 320) return t;
    if (hasRichAiGuide) {
      const alt = parasEn[i + 1] || parasEn[i + 2] || '';
      const altClean = densifyPage(alt, [...parasEn, fallbackDescEn], 700);
      if (altClean && altClean !== page1En && altClean.length >= 120) return altClean;
      return densifyPage(
        buildBody(
          titleEn,
          SLIDE_LABELS_EN[i] || 'details',
          parasEn[Math.min(i + 1, Math.max(0, parasEn.length - 1))] || fallbackDescEn || titleEn,
        ),
        [...parasEn, fallbackDescEn],
        700,
      );
    }
    if (i === 1) {
      return densifyPage(
        ...
        [...parasEn, fallbackDescEn, page1En],
        700,
      );
    }
    return '';
  ...
  const ranked = rankImagesForHero(
    input.images.filter((u) => !isJunkLandmarkPhotoUrl(u) || isDocumentaryDamagePhotoUrl(u)),
  );
  const attractive = ranked.filter((u) => !isUnattractiveLandmarkPhotoUrl(u) && !isJunkLandmarkPhotoUrl(u));
  const documentary = uniquePhotoList(
    input.images.filter((u) => isDocumentaryDamagePhotoUrl(u) && !isNonPhotoJunkUrl(u)),
  );
  ...
  const heroUri =
    (typeof input.heroUri === 'string' && input.heroUri.trim()) ||
    attractive[0] ||
    poolForStory.find((u) => !isUnattractiveLandmarkPhotoUrl(u)) ||
    '';
  if (heroUri) usedPhotos.add(heroUri);
  ...
  let compareOldUri =
    (typeof input.compareOldUri === 'string' && input.compareOldUri.trim()) || '';
  let compareNew =
    (typeof input.compareNewUri === 'string' && input.compareNewUri.trim()) || '';
  if (!compareOldUri || !compareNew || compareOldUri === compareNew) {
    ...
    if (!compareOldUri) compareOldUri = pairOld;
    if (!compareNew) compareNew = pairNew || '';
  ...
  if (!compareNew) compareNew = heroUri;
  if (compareOldUri) usedPhotos.add(compareOldUri);
  if (compareNew) usedPhotos.add(compareNew);
  ...
  const introPagesUk = bodiesUk.map((body, i) => {
    ...
    const preferred =
      i === 0 && typeof input.page1PhotoUri === 'string' && input.page1PhotoUri.trim()
        ? input.page1PhotoUri.trim()
        : '';
    const photoUri = takeNextUniquePhoto(preferred, fillPools, usedPhotos);
    const wantsSecondary =
      Number((layout as any).secondaryHeroHeightRatio) > 0 ||
      Number((layout as any).secondaryHeroHeightMax) > 0;
  ...
  const introPagesEn = bodiesEn.map((body, i) => {
    ...
    if (i === 1 && (ukPage as any).compareBeforeUri && (ukPage as any).compareAfterUri) {
      return {
        body,
        compareBeforeUri: (ukPage as any).compareBeforeUri,
        compareAfterUri: (ukPage as any).compareAfterUri,
        ...COMPARE_PAGE3_LAYOUT,
        ...layout,
        heroFit: 'cover',
      };
    }
    const photoUri = typeof (ukPage as any).photoUri === 'string' ? (ukPage as any).photoUri : '';
    const secondaryPhotoUri =
      typeof (ukPage as any).secondaryPhotoUri === 'string'
        ? (ukPage as any).secondaryPhotoUri
        : '';
    return {
      body,
      ...(photoUri ? { photoUri } : {}),
  ...
  const page1Photo =
    (typeof (introPagesUk[0] as any)?.photoUri === 'string' && (introPagesUk[0] as any).photoUri) ||
    takeNextUniquePhoto(
      typeof input.page1PhotoUri === 'string' ? input.page1PhotoUri : '',
      fillPools,
      usedPhotos,
  ...
  const shortIntroUk = cleanWikiPlainText(
    guide?.shortIntroUk || parasUk[0] || input.descUk || titleUk,
  ).slice(0, 500);
  const shortIntroEn = cleanWikiPlainText(
    guide?.shortIntroEn || parasEn[0] || input.descEn || titleEn,
  ).slice(0, 500);
  ...
  const distinctMiniPreview = (
    ...
    for (const p of pool) {
      if (!p || norm(p) === norm(s)) continue;
      if (norm(s).startsWith(norm(p).slice(0, 50))) continue;
      const sentence = p.split(/(?<=[.!?…])\s+/)[0] || p;
      const clipped = sentence.slice(0, 180).trim();
      if (clipped && norm(clipped) !== norm(s)) return clipped;
    }
    return '';
  };

  const guideQuiz = guide?.quiz && typeof guide.quiz === 'object' ? guide.quiz : null;
  const questionsFromGuide: NonNullable<ReturnType<typeof normalizeQuizQuestionPack>>[] = [];
  if (Array.isArray((guideQuiz as any)?.questions)) {
    for (const q of (guideQuiz as any).questions) {
      const n = normalizeQuizQuestionPack(q);
      if (n) questionsFromGuide.push(n);
      if (questionsFromGuide.length >= 3) break;
    }
  }
  if (!questionsFromGuide.length) {
    const legacy = normalizeQuizQuestionPack(guideQuiz);
    if (legacy) questionsFromGuide.push(legacy);
  }
  ...
  const quizQuestions: NonNullable<ReturnType<typeof normalizeQuizQuestionPack>>[] = [];
  const seenQuiz = new Set<string>();
  for (const q of [...questionsFromGuide, ...fallbackQs]) {
    if (!q) continue;
    const fp = `${q.questionUk}|${q.questionEn}`.toLowerCase().slice(0, 80);
    if (seenQuiz.has(fp)) continue;
    seenQuiz.add(fp);
    quizQuestions.push(q);
    if (quizQuestions.length >= 3) break;
  }
  if (!quizQuestions.length) {
    ...
    if (emergency) quizQuestions.push(emergency);
  }
  const quiz = {
    ...quizQuestions[0],
...
function pickClosingWowFact(candidates: string[]) {
  const scored = (candidates || [])
    ...
    .map((t) => {
      ...
      if (/\b(1[0-9]{3}|20[0-2][0-9])\b/.test(first)) score += 40;
      if (/\*\*[^*]{4,}\*\*/.test(first)) score += 30;
      if (/метр|meter|тонн|тон|шпиль|spire|орган|organ|вітраж|stained|обстріл|shell|ракет/i.test(first))
        score += 35;
      if (/матеріал зібрано|wikipedia|джерела та контекст|sources and context/i.test(first)) score -= 200;
...
function emptyItemTrace(index: number, item: ImportItem): EnrichItemTrace {
  return {
    index,
    name: item.name,
...
export async function enrichLocationsFromVerifiedSources(input: EnrichInput) {
  ...
  const emit = async (partial: Omit<EnrichLogEntry, 'ts'>) => {
    await input.onEvent?.({ ...partial, ts: nowIso() });
  };

  const wasRemoved = (itemIndex: number) => input.shouldSkipItem?.(itemIndex) === true;

  const abortIfRemoved = async (itemIndex: number, itemName: string, trace: EnrichItemTrace) => {
    if (!wasRemoved(itemIndex)) return false;
  ...
  for (let i = 0; i < items.length; i += 1) {
    const item = items[i];
    if (await abortIfRemoved(i, item.name, emptyItemTrace(i, item))) continue;
    ...
    try {
      if (await abortIfRemoved(i, item.name, trace)) continue;
      ...
      if (!ukHit && enHit) {
        ...
        if (!ukHit) ukHit = await resolveWikiHitForPlace({ ...placeCtx, lang: 'uk' });
      }
      ...
      await emit({
        ...
        itemIndex: i,
        itemName: item.name,
        message: `Текст: UK ${extractUk.length} симв., EN ${extractEn.length} симв.${
          trace.lat != null ? ` · coords ${trace.lat}, ${trace.lng}` : ''
        }`,
        data: {
          titleUk,
          titleEn,
      ...
      if (!extractUk && !extractEn) {
        ...
        if (trace.lat == null) trace.lat = alt.lat;
        if (trace.lng == null) trace.lng = alt.lng;
      ...
      if (!resolvedStreetAddress) {
        ...
        if (osmAddr?.lat != null && trace.lat == null) trace.lat = osmAddr.lat;
        if (osmAddr?.lng != null && trace.lng == null) trace.lng = osmAddr.lng;
        if (osmAddr?.url) altSources.push(osmAddr.url);
        await emit({
      ...
      const historicImages = await collectHistoricImages(
        item,
        city,
        titleEn,
        titleUk,
      ...
      const imagesWithHistoric = [...historicImages, ...images, ...documentaryImages].filter(
      ...
      const commonsHero =
        imagesForStory.find((u) => !isUnattractiveLandmarkPhotoUrl(u) && !aiAll.includes(u)) ||
      ...
      const personMentions = await withTimeout(
        resolvePersonMentions(guide?.people, emit, i, item.name),
        50000,
        [],
        'person-mentions',
      );
      ...
      if (heroUri) trace.thumbUri = heroUri;
      await input.onItemTrace?.(trace);
      ...
      const thumbUri =
        heroUri ||
        (story._meta && typeof story._meta === 'object' && (story._meta as any).thumbUri) ||
        imagesForStory[0] ||
        '';
      const visitCategory = inferVisitCategoryFromTexts(
        finalTitleUk,
        finalTitleEn,
        item.name,
        item.address,
        guide?.descUk,
        guide?.descEn,
        extractUk?.slice(0, 400),
        extractEn?.slice(0, 400),
      );
      ...
      let landmark: any = {
        ...
        descUk: cleanWikiPlainText(
          guide?.descUk || splitParagraphs(extractUk)[0] || extractUk || item.address || finalTitleUk,
        ).slice(0, 700),
        descEn: cleanWikiPlainText(
          guide?.descEn || splitParagraphs(extractEn)[0] || extractEn || item.address || finalTitleEn,
        ).slice(0, 700),
      ...
      if (rehostImages) {
        ...
        try {
          landmark = await withTimeout(
            rehostLandmarkMediaFields(landmark),
            180000,
            landmark,
            'rehost-media',
          );
        ...
        const hosted = Array.isArray(landmark.galleryUris)
          ? landmark.galleryUris.filter((u: string) => u && !/^https?:\/\/(upload\.wikimedia|commons)/i.test(u))
          : [];
        const anyHosted = Array.isArray(landmark.galleryUris)
          ? landmark.galleryUris.filter((u: string) => /^https?:\/\//i.test(u) && !/wikipedia|wikimedia/i.test(u))
          : [];
      ...
      try {
        landmark = await withTimeout(
          localizeLandmarkForAllAppLanguages(landmark, {
      ...
      const bodiesI18n = Array.isArray(landmark?.story?.introPagesBodiesI18n)
        ...
        : [];
      const pageLangCount =
        bodiesI18n[0] && typeof bodiesI18n[0] === 'object' ? Object.keys(bodiesI18n[0]).length : 0;
