...
async function ensureUniqueUsername(client: { query: Function }, base: string) {
  let candidate = base
    .slice(0, 28)
 ...
 */
export async function grantAdminByEmail(emailRaw: string): Promise<AdminListRow> {
  ...
  try {
    ...
    return out.rows[0] as AdminListRow;
  } catch (e) {
...
export async function revokeAdminByEmail(emailRaw: string, actorUserId: string): Promise<{ ok: true }> {
  const email = String(emailRaw || '').trim().toLowerCase();
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new HttpError(400, 'invalid_email');
  }
  ...
  const row = target.rows[0] as { id: string; role: string; status: string };
  ...
  const admins = await pool.query(
    `SELECT count(*)::int AS n FROM users WHERE role = 'admin' AND status <> 'deleted'`,
  );
