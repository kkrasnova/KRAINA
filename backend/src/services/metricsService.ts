...
const BOOTSTRAP_PHASE_NAMES = new RegExp(
  '^(first_screen|navigation_container_ready|home_tabs_mounted|feed_interactive|map_interactive)$',
);
...
export async function insertMetrics(
  ...
  try {
    await pool.query(
      `INSERT INTO app_metrics (user_id, session_id, app_version, platform, os_version, device_model, metrics, client_ts)
    ...
    if (Array.isArray(payload.metrics)) {
      for (const entry of payload.metrics) {
        if (entry && typeof entry === 'object') {
          alertIfSlowMetric(entry as Record<string, unknown>);
        }
      }
    }
  } catch (e) {
