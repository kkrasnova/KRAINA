...
export function landmarkGuideUsesClaude() {
  ...
  if (!claudeConfig.apiKey) return false;
...
export function landmarkImageProviderOrder(): Array<'gemini' | 'openai'> {
  const p = geminiConfig.imageProvider;
  if (p === 'openai') return ['openai'];
  if (p === 'gemini') return ['gemini'];
  // auto
  if (geminiConfig.apiKey) return ['gemini', 'openai'];
  return ['openai'];
}
...
export const config = {
  ...
  appVersion: (() => {
    try {
      const p = _require('../../package.json') as { version?: string };
  ...
  storageProvider: (process.env.STORAGE_PROVIDER ?? 'local').trim().toLowerCase() as
    | 'local'
    | 's3',
  ...
  maxLandmarkBundleJsonBytes: Number(
    process.env.MAX_LANDMARK_BUNDLE_BYTES ?? 15 * 1024 * 1024,
  ),
