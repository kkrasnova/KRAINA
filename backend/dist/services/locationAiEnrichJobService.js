...
import { enrichLocationsFromVerifiedSources, } from './locationAiEnrichmentService.js';
import { getLandmarkContentBundle, hasLandmarkContentBundle, saveLandmarkContentBundle, } from './landmarkContentAdminService.js';
import { publishLandmarkBundleToFirestore } from './landmarkContentFirestorePublisher.js';
import { dedupeCityRegionsInBundle, resolveCanonicalCity, } from './cityRegionCanonical.js';
    ...
    try {
        ...
        for (const row of list) {
            ...
            const job = {
                ...
                duplicateDecisions: row.duplicateDecisions && typeof row.duplicateDecisions === 'object'
                    ? row.duplicateDecisions
                    : {},
                liveSnapshot: row.liveSnapshot && typeof row.liveSnapshot === 'object' ? row.liveSnapshot : undefined,
            };
...
function unionSnapshots(primary, secondary) {
    const next = dedupeCityRegionsInBundle(JSON.parse(JSON.stringify(primary && typeof primary === 'object' ? primary : emptySnapshot())));
    if (!secondary || typeof secondary !== 'object' || Array.isArray(secondary))
        return next;
    if (!next.regions || typeof next.regions !== 'object')
        next.regions = {};
    if (!Array.isArray(next.homeCountryOrder))
        next.homeCountryOrder = [];
    ...
    for (const raw of secOrder) {
        ...
        if (cid && !next.homeCountryOrder.includes(cid))
            next.homeCountryOrder.push(cid);
    }
    const secMap = secondary.homeRegionIdsByCountry && typeof secondary.homeRegionIdsByCountry === 'object'
    ...
    for (const [rid, src] of Object.entries(secRegions)) {
        ...
        const existingIds = new Set(dst.landmarks.map((lm) => String(lm?.id || '')));
        const existingTitles = new Set(dst.landmarks
            .flatMap((lm) => [normalizeTextKey(lm?.titleUk), normalizeTextKey(lm?.titleEn)])
            .filter(Boolean));
        for (const lm of Array.isArray(src.landmarks) ? src.landmarks : []) {
            ...
            if (lm.id)
                existingIds.add(String(lm.id));
            if (titleKey)
                existingTitles.add(titleKey);
        }
    }
    for (const heroKey of ['homeCountryHeroRefs', 'homeCountryHeroUris']) {
        if (!next[heroKey] || typeof next[heroKey] !== 'object')
            next[heroKey] = {};
        const secHero = secondary[heroKey];
        if (secHero && typeof secHero === 'object') {
            for (const [k, v] of Object.entries(secHero)) {
                if (v != null && next[heroKey][k] == null)
                    next[heroKey][k] = v;
            }
        }
    }
...
function findSimilarLandmark(landmarks, candidate) {
    ...
    for (let i = 0; i < list.length; i += 1) {
        ...
        let score = 0;
        let reason = '';
        if (titlesLookSimilar(candidate?.titleUk, lm.titleUk) ||
...
function mergeLandmarkFields(existing, incoming) {
...
function mergeLandmarksIntoSnapshot(base, mergeTarget, landmarks, opts) {
    const next = dedupeCityRegionsInBundle(JSON.parse(JSON.stringify(base && typeof base === 'object' ? base : emptySnapshot())));
    if (!next.regions || typeof next.regions !== 'object')
        next.regions = {};
    if (!Array.isArray(next.homeCountryOrder))
        next.homeCountryOrder = [];
    ...
    if (!countryId)
        throw new HttpError(400, 'country_required');
    if (!next.homeCountryOrder.includes(countryId))
        next.homeCountryOrder.push(countryId);
    ...
    const existingIds = new Set(region.landmarks.map((lm) => String(lm?.id || '')));
    const pending = [];
    ...
    for (let idx = 0; idx < landmarks.length; idx += 1) {
        ...
        if (!region.heroUri && lm?.thumbUri)
            region.heroUri = lm.thumbUri;
...
async function resolvePublishBase(input) {
    let disk = null;
    ...
    const client = input.snapshot && typeof input.snapshot === 'object' && !Array.isArray(input.snapshot)
        ? input.snapshot
        : null;
    // Always prefer disk as primary so a thin CMS editor never wipes Europe.
    if (disk)
        return unionSnapshots(disk, client);
    if (client)
        return dedupeCityRegionsInBundle(JSON.parse(JSON.stringify(client)));
    ...
    try {
        const { landmarks } = await enrichLocationsFromVerifiedSources({
            ...
            onLandmarkReady: async (landmark, itemIndex) => {
                if (!Array.isArray(job.landmarks))
                    job.landmarks = [];
                ...
                if (input.autoPublish && input.mergeTarget && liveSnapshot) {
                    ...
                    const { snapshot, regionId, pending } = mergeLandmarksIntoSnapshot(liveSnapshot, input.mergeTarget, [landmark], {
                        actions: [decided],
        ...
        if (input.autoPublish && landmarks.length && !job.publishedCount && !(job.pendingDuplicates || []).length) {
            ...
            const { snapshot, regionId, pending } = mergeLandmarksIntoSnapshot(base, input.mergeTarget, landmarks, {
                actions,
                defaultAction: 'ask',
...
export function getEnrichJob(jobId) {
    loadJobsFromDisk();
    return jobs.get(String(jobId || '')) || null;
}
export function createEnrichJob(input) {
    loadJobsFromDisk();
    pruneJobs();
...
export function removeEnrichJobItem(jobId, itemIndex) {
    loadJobsFromDisk();
    ...
    if (!Number.isFinite(idx) || idx < 0)
        throw new HttpError(400, 'invalid_item_index');
    if (!Array.isArray(job.cancelledIndexes))
        job.cancelledIndexes = [];
    if (!job.cancelledIndexes.includes(idx))
        job.cancelledIndexes.push(idx);
    if (Array.isArray(job.pendingDuplicates)) {
...
export async function resolveEnrichJobDuplicate(jobId, itemIndex, action) {
    loadJobsFromDisk();
    const job = jobs.get(String(jobId || ''));
    if (!job)
        throw new HttpError(404, 'job_not_found');
    ...
    if (pendingIdx < 0)
        throw new HttpError(404, 'duplicate_not_pending');
    ...
    if (action === 'skip') {
        ...
        const { snapshot, regionId } = mergeLandmarksIntoSnapshot(liveSnapshot, input.mergeTarget, [pending.landmark], {
            actions: [action],
...
export function serializeEnrichJobSummary(job) {
    ...
    okTraces.forEach((t, i) => {
        if (landmarks[i])
            landmarkByIndex.set(t.index, landmarks[i]);
    });
    // Prefer match by title
    for (const lm of landmarks) {
        const key = normalizeTextKey(lm?.titleUk || lm?.titleEn);
        const tr = (job.itemTraces || []).find((t) => t.status === 'ok' && normalizeTextKey(t.name) === key);
        if (tr)
            landmarkByIndex.set(tr.index, lm);
    }
    return {
        ok: true,
        id: job.id,
        ...
        retry: {
            ...
            itemsAll: (Array.isArray(input.items) && input.items.length
                ? input.items
                : (job.itemTraces || []).map((t) => ({ name: t.name, address: t.address || '' }))).map((it) => ({
...
export function serializeEnrichJob(job, opts) {
...
function landmarksPresent(job) {
    return Array.isArray(job.landmarks) && job.landmarks.length > 0;
}
//# sourceMappingURL=locationAiEnrichJobService.js.map