...
export function splitTextForTts(text, maxBytes) {
...
async function googleSynthesizeChunk(text, contentLang) {
    ...
    if (!json.audioContent)
        throw new HttpError(502, 'tts_empty_audio');
    return Buffer.from(json.audioContent, 'base64');
}
async function openAiSynthesizeChunk(text, contentLang) {
...
export async function synthesizeLandmarkSpeech(text, rawLang) {
    const contentLang = normalizeContentLang(rawLang);
    const trimmed = String(text || '').trim();
    if (!trimmed)
        throw new HttpError(400, 'tts_empty_text');
    if (trimmed.length > 20_000)
        throw new HttpError(400, 'tts_text_too_long');
...
//# sourceMappingURL=landmarkTtsService.js.map