...
function interestFilter(row, interests) {
    if (!interests)
        return true;
    const anyOn = interests.landmark === true ||
        interests.park === true ||
    ...
    if (interests.museum && c === 'museum')
        return true;
    if (interests.park && c === 'park')
        return true;
    if (interests.landmark && (c === 'monument' || c === 'church' || c === 'art'))
        return true;
    if (interests.cafe && /café|cafe|coffee|ресторан/i.test(row.title))
        return true;
    if (interests.cafe && c === 'other')
        return true;
    if (interests.architecture &&
...
function thumbForClient(row) {
    const u = row.cover_image_url?.trim();
    if (u && /^https?:\/\//i.test(u))
        return { uri: u };
    return null;
}
function budgetPreferenceText(tier) {
    switch (tier) {
        case 'free':
...
function parseAssistantJson(content) {
    ...
    try {
        ...
        if (!Array.isArray(j.orderedIds))
            return null;
        const orderedIds = j.orderedIds.filter((x) => typeof x === 'string' && x.length > 0);
...
async function callChatCompletions(system, user) {
    ...
    if (!apiKey)
        throw new HttpError(503, 'ai_not_configured', 'Set OPENAI_API_KEY or use heuristic-only catalog.');
    ...
    if (!content || typeof content !== 'string')
        throw new HttpError(502, 'ai_empty_content');
    return content;
}
function greedyOrder(pool, budgetMin, transport, userOrigin) {
    if (!pool.length)
        return [];
...
function boundsMapRegion(stops) {
    if (!stops.length) {
...
export async function suggestAiRoute(input) {
        ...
        try {
            ...
            if (parsed?.orderedIds?.length) {
                ...
                for (const id of parsed.orderedIds) {
                    ...
                    if (row && !ordered.find((x) => x.id === row.id))
                        ordered.push(row);
                }
                if (ordered.length >= 2) {
                    usedAi = true;
    ...
    let totalKm = 0;
    if (userOrigin && ordered[0])
        totalKm += haversineKm(userOrigin, ordered[0]);
    for (let i = 1; i < ordered.length; i += 1)
        totalKm += haversineKm(ordered[i - 1], ordered[i]);
    let totalMinutes = 0;
    let prev = userOrigin ?? null;
    for (const s of ordered) {
        if (prev)
            totalMinutes += travelMinutes(prev, s, transport);
...
//# sourceMappingURL=aiRouteSuggestService.js.map