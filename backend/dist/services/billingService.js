...
function planTypeFromProductId(productId, platform) {
    const { apple, google } = skuSets();
    const m = platform === 'apple' ? apple : google;
    if (m.explorer.has(productId))
        return 'explorer';
    if (m.family.has(productId))
        return 'family';
    if (m.pro.has(productId))
        return 'pro';
    throw new HttpError(400, 'unknown_product_id');
}
function assertBillingSkusConfigured(platform) {
    const { apple, google } = skuSets();
    const s = platform === 'apple' ? apple : google;
    const any = s.explorer.size + s.pro.size + s.family.size > 0;
    if (!any) {
        throw new HttpError(503, 'billing_sku_not_configured');
    }
}
async function postAppleVerify(url, receiptBase64, password) {
...
function parseAppleLatestReceipt(rows, hintProductId) {
    const list = (rows || []).filter(Boolean);
    const now = Date.now();
    const active = list.filter((r) => {
        if (!r.product_id || !r.expires_date_ms)
            return false;
        if (r.cancellation_date_ms)
            return false;
    ...
    if (hintProductId) {
        ...
        pick = hinted.sort((a, b) => Number(b.expires_date_ms || 0) - Number(a.expires_date_ms || 0))[0];
    }
    if (!pick) {
        pick = active.sort((a, b) => Number(b.expires_date_ms || 0) - Number(a.expires_date_ms || 0))[0];
    }
...
export async function verifyAppleReceipt(receiptBase64, hintProductId) {
...
async function googlePublisherAccessToken() {
...
export async function verifyGoogleSubscription(productId, purchaseToken) {
    ...
    const url = `https://androidpublisher.googleapis.com/androidpublisher/v3/applications/${encodeURIComponent(pkg)}/purchases/subscriptions/${encodeURIComponent(productId)}/tokens/${encodeURIComponent(purchaseToken)}`;
    const res = await fetch(url, {
...
async function upsertUserSubscription(userId, params) {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        await client.query(`UPDATE subscriptions SET is_active = false WHERE user_id = $1`, [userId]);
        await client.query(`INSERT INTO subscriptions (user_id, plan_type, billing_period, price_usd, starts_at, expires_at, is_active, payment_provider, external_id)
       VALUES ($1, $2, 'monthly', NULL, now(), $3, true, $4, $5)`, [userId, params.planType, params.expiresAt.toISOString(), params.paymentProvider, params.externalId]);
...
export async function verifyAndSaveSubscription(userId, body) {
