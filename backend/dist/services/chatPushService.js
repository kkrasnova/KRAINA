 ...
 */
async function getExpoPushToken(receiverId) {
    const tokenRow = await pool.query(`SELECT expo_push_token FROM push_tokens WHERE user_id = $1::uuid AND expo_push_token IS NOT NULL`, [receiverId]);
    if (!tokenRow.rowCount)
        return null;
    const expoToken = String(tokenRow.rows[0].expo_push_token);
 ...
 */
async function sendExpoPushRaw(receiverId, payload) {
    ...
    try {
        ...
        if (data && data.status === 'error') {
            ...
            if (details?.error === 'DeviceNotRegistered') {
                await pool.query(`UPDATE push_tokens SET expo_push_token = NULL WHERE user_id = $1::uuid`, [receiverId]);
 ...
 * Fire-and-forget: logs errors but never throws.
 */
export async function sendChatMessagePush(receiverId, payload) {
 ...
 * Fire-and-forget: logs errors but never throws.
 */
export async function sendPostLikePush(postAuthorId, payload) {
 ...
 * Fire-and-forget: logs errors but never throws.
 */
export async function sendPostCommentPush(postAuthorId, payload) {
 ...
 * Fire-and-forget: logs errors but never throws.
 */
export async function sendCommentLikePush(commentAuthorId, payload) {
 ...
 */
export async function registerExpoPushToken(userId, expoToken) {
    const token = String(expoToken || '').trim();
    if (!token || !token.startsWith('ExponentPushToken[')) {
        throw new Error('invalid_expo_push_token');
    }
    await pool.query(`INSERT INTO push_tokens (user_id, expo_push_token, device_family, updated_at)
     VALUES ($1::uuid, $2, $3, now())
     ON CONFLICT (user_id)
     DO UPDATE SET expo_push_token = EXCLUDED.expo_push_token,
                   device_family = EXCLUDED.device_family,
                   updated_at = now()`, [userId, token, 'expo']);
}
/**
 * Remove the Expo push token for a user (on logout).
 */
export async function removeExpoPushToken(userId) {
    await pool.query(`UPDATE push_tokens SET expo_push_token = NULL, updated_at = now() WHERE user_id = $1::uuid`, [userId]);
}
//# sourceMappingURL=chatPushService.js.map