...
export function resolveCanonicalCity(input) {
    ...
    for (const key of candidates) {
        ...
        if (hit)
            return { id: hit.id, titleUk: hit.titleUk, titleEn: hit.titleEn };
    }
 ...
 */
export function findExistingRegionId(regions, canonical) {
    if (!regions || typeof regions !== 'object')
        return '';
    if (regions[canonical.id])
        return canonical.id;
 ...
 */
export function dedupeCityRegionsInBundle(bundle) {
    ...
    for (const [from, to] of Object.entries(rename)) {
        ...
        if (!next.regions[to]) {
            ...
            const existingTitles = new Set(dst.landmarks.flatMap((lm) => [norm(lm?.titleUk), norm(lm?.titleEn)]).filter(Boolean));
            ...
            if (!dst.heroUri && src.heroUri)
                dst.heroUri = src.heroUri;
            if (!dst.center?.latitude && src.center?.latitude)
    ...
    for (const countryId of Object.keys(next.homeRegionIdsByCountry)) {
        const ids = Array.isArray(next.homeRegionIdsByCountry[countryId])
            ...
            : [];
        const mapped = ids.map((id) => rename[id] || id);
        next.homeRegionIdsByCountry[countryId] = [...new Set(mapped)].filter((id) => next.regions[id]);
    }
    return next;
}
//# sourceMappingURL=cityRegionCanonical.js.map