...
export async function listPublishedLocations(opts) {
...
export async function searchPublishedLocationsFTS(q, limit) {
    const trimmed = q.trim().slice(0, 200);
    if (trimmed.length < 2)
        return [];
    const lim = Math.min(50, Math.max(1, Math.floor(limit)));
    const rows = await sequelize.query(`SELECT id, title, city, country, lat, lng, category, cover_image_url,
     ...
     LIMIT :lim`, { replacements: { q: trimmed, lim }, type: QueryTypes.SELECT });
    return rows.map((r) => ({
        id: r.id,
        title: r.title,
        city: r.city,
...
//# sourceMappingURL=locationsCrudService.js.map