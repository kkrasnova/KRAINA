...
import pg from 'pg';
import { registerSchema, loginSchema, googleSchema, appleSchema, refreshSchema, forgotPasswordSchema, resetPasswordSchema, } from '../schemas/auth.schemas.js';
import { registerUser, loginWithPassword, loginOrRegisterGoogle, loginOrRegisterApple, rotateRefreshToken, logoutAllRefreshTokens, requestPasswordReset, resetPasswordWithToken, } from '../services/authService.js';
...
function zodFirstCode(err) {
    ...
    if (i?.message === 'weak_password' ||
        i?.message === 'invalid_email' ||
        i?.message === 'invalid_username') {
        return i.message;
    }
    if (i?.path.includes('username'))
        return 'invalid_username';
    if (i?.path.includes('email'))
