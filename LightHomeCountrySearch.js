 ...
 */
export default function LightHomeCountrySearch({
  variant = 'light',
  placeholder,
  value,
  onChangeText,
  ...
  const localRows = useMemo(() => {
    ...
    // Пустой запрос: полный каталог. С текстом: только реальные совпадения поиска.
    if (!queryHasChars) return buildHomeBrowseRows(language);
    return buildHomeSearchRows(value, language);
  }, [peopleOnlyMode, queryHasChars, value, language]);
  ...
  useEffect(() => {
    if (!profileSearchEnabled || !showProfiles) {
      setProfileHits([]);
      return;
    }
