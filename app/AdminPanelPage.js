...
export default function AdminPanelPage({ navigation, route }) {
  ...
  const handleImportJson = useCallback((mode) => {
    const raw = (importJsonTextRef.current || '').trim();
    if (!raw) {
      Alert.alert('Помилка', 'Введіть або вставте рядок JSON.');
      return;
    }
    try {
      const parsed = JSON.parse(raw);
      if (!parsed || typeof parsed !== 'object') {
        throw new Error('JSON не є об\'єктом.');
      }
      if (!parsed.regions || typeof parsed.regions !== 'object') {
        throw new Error('Відсутній або некоректний об\'єкт "regions" у корені.');
      }
      if (!Array.isArray(parsed.homeCountryOrder)) {
        throw new Error('Відсутній або некоректний масив "homeCountryOrder" у корені.');
      }
      if (!parsed.homeRegionIdsByCountry || typeof parsed.homeRegionIdsByCountry !== 'object') {
        throw new Error('Відсутній або некоректний об\'єкт "homeRegionIdsByCountry" у корені.');
      }

      setDraft((prev) => {
        const next = clone(prev);
        if (mode === 'overwrite') {
          next.homeCountryOrder = [...parsed.homeCountryOrder];
          next.homeRegionIdsByCountry = {};
          for (const [k, v] of Object.entries(parsed.homeRegionIdsByCountry)) {
            next.homeRegionIdsByCountry[k] = Array.isArray(v) ? [...v] : [];
          }
          next.regions = {};
          for (const [k, v] of Object.entries(parsed.regions)) {
            if (v && typeof v === 'object') next.regions[k] = clone(v);
          }
          next.homeCountryHeroRefs = parsed.homeCountryHeroRefs ? clone(parsed.homeCountryHeroRefs) : {};
          next.homeCountryHeroUris = parsed.homeCountryHeroUris ? clone(parsed.homeCountryHeroUris) : {};
        } else {
          // Merge
          parsed.homeCountryOrder.forEach((cid) => {
            if (!next.homeCountryOrder.includes(cid)) {
              next.homeCountryOrder.push(cid);
            }
          });
          for (const [cid, rids] of Object.entries(parsed.homeRegionIdsByCountry)) {
            if (!next.homeRegionIdsByCountry[cid]) {
              next.homeRegionIdsByCountry[cid] = [];
            }
            if (Array.isArray(rids)) {
              rids.forEach((rid) => {
                if (!next.homeRegionIdsByCountry[cid].includes(rid)) {
                  next.homeRegionIdsByCountry[cid].push(rid);
                }
              });
            }
          }
          for (const [rid, rdata] of Object.entries(parsed.regions)) {
            if (rdata && typeof rdata === 'object') {
              next.regions[rid] = { ...(next.regions[rid] || {}), ...clone(rdata) };
            }
          }
          if (parsed.homeCountryHeroRefs && typeof parsed.homeCountryHeroRefs === 'object') {
            next.homeCountryHeroRefs = { ...(next.homeCountryHeroRefs || {}), ...clone(parsed.homeCountryHeroRefs) };
          }
          if (parsed.homeCountryHeroUris && typeof parsed.homeCountryHeroUris === 'object') {
            next.homeCountryHeroUris = { ...(next.homeCountryHeroUris || {}), ...clone(parsed.homeCountryHeroUris) };
          }
        }
        return next;
      });

      const countCountries = parsed.homeCountryOrder.length;
      const countRegions = Object.keys(parsed.regions).length;
      Alert.alert(
        'Успішно',
        `Дані імпортовано! Додано/оновлено країн: ${countCountries}, регіонів: ${countRegions}.\n\nНе забудьте натиснути кнопку "Зберегти" внизу сторінки для перманентного запису.`
      );
      setImportJsonText('');
      importJsonTextRef.current = '';
    } catch (e) {
      Alert.alert('Помилка імпорту', `Некоректний формат JSON: ${e.message}`);
    }
  }, []);
  ...
  const handleLoadSampleJson = useCallback(() => {
          ...
          landmarks: [
            {
              ...
              minutes: 60,
              free: false,
              descUk: 'Визначний архітектурний пам\'ятник православної церкви.',
              descEn: 'A prominent architectural monument of the Orthodox Church.',
            }
          ]
        }
      },
    ...
    importJsonTextRef.current = JSON.stringify(sample, null, 2);
  }, []);
  ...
  return (
    <View style={[styles.root, { backgroundColor: isLight ? LIGHT_BAR_BG : APP_SCREEN_BG }]}>
      ...
      >
        ...
        <View style={[styles.aiAdminCard, { borderColor: border, backgroundColor: cardBg, marginTop: 16 }]}>
          <Text style={[styles.workflowTitle, { color: textMain }]}>Локації: Імпорт / Експорт JSON</Text>
          <Text style={[styles.sectionHint, { color: muted }]}>
            Керуйте базою даних локацій за допомогою структурованого JSON-файлу.
          </Text>

          <View style={styles.jsonTabContainer}>
            <Pressable
              onPress={() => setJsonTab('import')}
              style={[
                styles.jsonTabButton,
                jsonTab === 'import' && { backgroundColor: accent, borderColor: accent }
              ]}
              android_ripple={ripple}
            >
              <Text style={[styles.jsonTabText, { color: jsonTab === 'import' ? onAccentButtonText(isLight) : textMain }]}>
                Імпорт JSON
              </Text>
            </Pressable>
            <Pressable
              onPress={() => setJsonTab('export')}
              style={[
                styles.jsonTabButton,
                jsonTab === 'export' && { backgroundColor: accent, borderColor: accent }
              ]}
              android_ripple={ripple}
            >
              <Text style={[styles.jsonTabText, { color: jsonTab === 'export' ? onAccentButtonText(isLight) : textMain }]}>
                Експорт JSON
              </Text>
            </Pressable>
          </View>
          ...
          {jsonTab === 'import' ? (
            <View style={{ marginTop: 12 }}>
              <Text style={[styles.miniLabel, { color: muted, marginBottom: 6, fontWeight: '700' }]}>
                Вставте рядок JSON:
              </Text>
              <TextInput
                multiline
                ...
                onChangeText={(t) => { setImportJsonText(t); importJsonTextRef.current = t; }}
                placeholder='{"homeCountryOrder": ["UA"], ...}'
                placeholderTextColor={muted}
                style={[
                  styles.jsonInput,
                  {
              ...
              />
              <View style={[styles.rowBtns, { marginTop: 12, flexWrap: 'wrap', gap: 8 }]}>
                <Pressable
                  onPress={() => handleImportJson('merge')}
                  style={[styles.jsonActionBtn, { backgroundColor: accent }]}
                  android_ripple={ripple}
                >
                  <Text style={{ color: onAccentButtonText(isLight), fontWeight: '700', fontSize: 13 }}>
                    Об'єднати зміни
                  </Text>
                </Pressable>
                <Pressable
                  onPress={() => handleImportJson('overwrite')}
                  style={[styles.jsonActionBtn, { backgroundColor: '#EB4335' }]}
                  android_ripple={ripple}
                >
                ...
                <Pressable
                  ...
                  style={[styles.jsonActionBtnOutline, { borderColor: accent }]}
                  android_ripple={ripple}
                >
            ...
            <View style={{ marginTop: 12 }}>
              <Text style={[styles.miniLabel, { color: muted, marginBottom: 6, fontWeight: '700' }]}>
                Поточна конфігурація (можна виділити та скопіювати):
              </Text>
              <TextInput
                multiline
                editable={true}
              ...
              />
              <Text style={[styles.sectionHint, { color: muted, marginTop: 8 }]}>
                💡 Натисніть на текстове поле, оберіть "Виділити все" та "Скопіювати", щоб зберегти ваші локації у файлі.
              </Text>
            </View>
          )}
        </View>
