...
export default function OnboardingIntroPage({ navigation, route }) {
  ...
  const onboardCopyScrollTopReserveRaw =
    onboardBannerBottomScreenY != null
      ...
      : 0;
  /**
   * Мобільний: «сирий» paddingTop часто > висоти зони `flex:1` (герой + компенсація translateY).
   * Тоді у внутрішнього View не лишається місця під колонку — текст не видно.
   */
  const onboardCopyInnerPadBottomPx =
    ONBOARD_COPY_BOTTOM_SAFE_PX + overflowScrollReliefBottomPx;
  const onboardFooterApproxPx = Math.round(Math.max(116, screenH * 0.14));
  const onboardCopyMinTextSlotPx = Math.round(Math.max(180, screenH * 0.22));
  const onboardCopyMaxTopReservePx =
      ...
      ? Math.max(
          ...
          Math.round(
            screenH -
              ...
              onboardFooterApproxPx -
              onboardCopyInnerPadBottomPx -
              onboardCopyMinTextSlotPx,
          ),
        )
...
const styles = StyleSheet.create({
  ...
  /** Мобільний: оверлей кроків 0…4 — вище декоративних смуг героя, інакше текст не видно. */
