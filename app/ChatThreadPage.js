...
export default function ChatThreadPage({ navigation, route }) {
  ...
  const renderMessage = useCallback(
    ({ item, index }) => {
      ...
      const showAvatar = !mine && (!prev || prev.fromMe);
      const avatarNode = showAvatar ? (
        peerAvatarUrl ? (
          <Image source={{ uri: peerAvatarUrl }} style={styles.tinyAvatar} resizeMode="cover" />
        ) : (
        ...
        <View style={[styles.bubbleRow, mine ? styles.rowEnd : styles.rowStart]}>
          ...
          >
            <Text
              style={[
                styles.bubbleText,
                { color: mine ? (isLight ? '#FFFFFF' : '#101010') : incomingText },
              ]}
            >
              {item.text}
            </Text>
    ...
    [language, navigation, shell, isLight, peerAvatarUrl, outgoingBg],
  );
  ...
  return (
    <View style={[styles.screen, { backgroundColor: bg }]}>
      {!isLight ? (
        <LinearGradient
          colors={['rgba(225,255,0,0.12)', 'rgba(225,255,0,0.02)', 'transparent']}
          start={{ x: 0, y: 0.8 }}
          end={{ x: 0.5, y: 0 }}
