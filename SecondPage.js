...
export default function SecondPage({ navigation }) {
  ...
  const handleContinue = async () => {
    ...
    if (PREVIEW_SELECT_COUNTRY_BEFORE_REGISTRATION) {
      navigation?.replace?.('SelectCountry', { language, previewBeforeAuth: true });
    } else {
      navigation?.replace?.('ThirdPage', { language });
    }
  };
...
const styles = StyleSheet.create({
  ...
  continueText: {
    ...
    backgroundColor: 'transparent',
    ...(Platform.OS === 'android'
      ? { fontFamily: 'sans-serif-medium', color: '#000000', includeFontPadding: false }
