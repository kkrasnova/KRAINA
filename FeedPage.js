...
function FeedHeader({ appTheme, insetsTop, onAdd, onMessages }) {
...
function PostMediaCarousel({ post, accent }) {
  ...
  if (media.length === 1) {
    ...
    return isVid ? (
      ...
      <Image source={{ uri: one }} style={styles.postImage} resizeMode="cover" />
    );
  }
  return (
    <View>
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return isVid ? (
            ...
            <Image source={{ uri: u }} style={styles.postImage} resizeMode="cover" />
          );
        }}
      />
      ...
      >
        ...
        {posts.map((post) => (
          ...
          >
            ...
            <PostMediaCarousel post={post} accent={accent} />
...
const styles = StyleSheet.create({
  ...
  /** Вкладки «Друзі / Світ» на всю ширину екрана, між історіями та стрічкою */
  feedTabsBleed: {
    marginHorizontal: -18,
    marginTop: 10,
    borderBottomWidth: StyleSheet.hairlineWidth,
