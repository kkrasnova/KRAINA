...
} from './khanenko-uk-source.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(__dirname, '..');
const OUT_PATH = path.join(ROOT, 'app', 'landmarkIntroI18n', 'khanenko.js');
const CACHE_PATH = path.join(ROOT, 'scripts', '.khanenko-intro-i18n-cache.json');

const TARGET_LANGS = ['en', 'de', 'pl', 'nl', 'es', 'lt', 'lv', 'ro', 'it', 'hy'];
const ALL_LANGS = ['uk', ...TARGET_LANGS];

const QUIZ_UK = {
  question: 'Скільки предметів налічують фонди Музею Ханенків?',
  multiHint: 'Пам’ятайте число зі слайду про фонди — воно згадується і на вступному, і на слайді «Цікавий факт».',
  options: [
    { text: '5 000', correct: false },
    { text: '25 000', correct: true },
    { text: '100 000', correct: false },
  ],
};

const TITLE_UK = 'Музей Ханенків';
const DESC_UK =
...
async function main() {
  ...
  process.stderr.write('Translating Khanenko intro i18n from Ukrainian…\n');

  const shortIntro = await fillRow(KHANENKO_SHORT_INTRO_UK, cache, stats, offline);
  saveCache(cache);
  const miniPreview = await fillRow(KHANENKO_MINI_PREVIEW_UK, cache, stats, offline);
  saveCache(cache);
  const introPage1 = await fillRow(KHANENKO_INTRO_PAGE1_UK, cache, stats, offline);
  saveCache(cache);
  ...
  const pageBodies = [];
  for (let i = 0; i < KHANENKO_INTRO_PAGE_BODIES_UK.length; i++) {
    process.stderr.write(`  page ${i + 2}/13\n`);
    pageBodies.push(await fillRow(KHANENKO_INTRO_PAGE_BODIES_UK[i], cache, stats, offline));
    saveCache(cache);
  }
  ...
  let js = `/** Auto-generated Khanenko intro translations — do not edit by hand. */\n\n`;
  js += `export const KHANENKO_INTRO_I18N = {\n`;
