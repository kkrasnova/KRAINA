...
} from './maidan-uk-source.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(__dirname, '..');
const OUT_PATH = path.join(ROOT, 'app', 'landmarkIntroI18n', 'maidan.js');
const CACHE_PATH = path.join(ROOT, 'scripts', '.maidan-intro-i18n-cache.json');

const TARGET_LANGS = ['en', 'de', 'pl', 'nl', 'es', 'lt', 'lv', 'ro', 'it', 'hy'];
const ALL_LANGS = ['uk', ...TARGET_LANGS];

const QUIZ_UK = {
  question: 'Що стоїть на вершині Монументу Незалежності на Майдані?',
  multiHint: 'Погляньте на білу колону в центрі площі — там дівчина з калиновою гілкою.',
  options: [
    { text: 'Оранта-Україна', correct: true },
    { text: 'Золотий Архангел Михаїл', correct: false },
    { text: 'Червона зірка', correct: false },
  ],
};

const TITLE_UK = 'Майдан Незалежності';
const DESC_UK = 'Центральна площа столиці — події, фонтани та прогулянки.';
...
async function main() {
  ...
  process.stderr.write('Translating Maidan intro i18n from Ukrainian…\n');

  const shortIntro = await fillRow(MAIDAN_SHORT_INTRO_UK, cache, stats, offline);
  saveCache(cache);
  const miniPreview = await fillRow(MAIDAN_MINI_PREVIEW_UK, cache, stats, offline);
  saveCache(cache);
  const introPage1 = await fillRow(MAIDAN_INTRO_PAGE1_UK, cache, stats, offline);
  saveCache(cache);
  const illustrationLink = await fillRow(MAIDAN_PAGE2_ILLUSTRATION_LINK_UK, cache, stats, offline);
  saveCache(cache);
  const illustrationCaption = await fillRow(MAIDAN_PAGE2_ILLUSTRATION_CAPTION_UK, cache, stats, offline);
  saveCache(cache);
  ...
  const pageBodies = [];
  for (let i = 0; i < MAIDAN_INTRO_PAGE_BODIES_UK.length; i++) {
    process.stderr.write(`  page ${i + 2}/13\n`);
    pageBodies.push(await fillRow(MAIDAN_INTRO_PAGE_BODIES_UK[i], cache, stats, offline));
    saveCache(cache);
  }
  ...
  let js = `/** Auto-generated Maidan intro translations — do not edit by hand. */\n\n`;
  js += `export const MAIDAN_INTRO_I18N = {\n`;
