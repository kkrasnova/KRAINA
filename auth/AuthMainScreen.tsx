...
const BG_LOGIN = require('../assets/228.png');
const BG_REGISTER = require('../assets/226-2.png');
...
export default function AuthMainScreen({ navigation, route }: Props) {
  ...
  const onRegister = async () => {
    ...
    const tryRegister = async (uname: string, retries = 0): Promise<void> => {
      try {
        await registerWithPassword(em, pwd, uname);
      } catch (e) {
  ...
  const bgShellStyle = [s.screen, { width: bgWidth, height: bgHeight, minWidth: bgWidth, minHeight: bgHeight }];
  const bgImageStyle = {
    width: bgWidth,
    height: bgHeight * 1.22,
    top: -bgHeight * 0.11,
    left: 0,
  };
  ...
  return (
    <ImageBackground
      source={tab === 'register' ? BG_REGISTER : BG_LOGIN}
      style={bgShellStyle}
          ...
          <ScrollView
            contentContainerStyle={[
              s.scroll,
              {
                ...
                paddingTop: Math.round(height * 0.31),
              },
            ]}
            keyboardShouldPersistTaps="always"
            nestedScrollEnabled
